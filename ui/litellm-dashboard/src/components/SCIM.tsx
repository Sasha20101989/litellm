...
const SCIMConfig: React.FC<SCIMConfigProps> = ({ accessToken, userID, proxySettings }) => {
  ...
  const handleCreateSCIMToken = async (values: any) => {
    if (!accessToken || !userID) {
      NotificationsManager.fromBackend(t("admin.scim.loginRequired"));
      return;
    }
    ...
    try {
      ...
      NotificationsManager.success(t("admin.scim.tokenCreated"));
    } catch (error: any) {
      console.error("Error creating SCIM token:", error);
      NotificationsManager.fromBackend(t("admin.scim.tokenFailed", { error: parseErrorMessage(error) }));
    } finally {
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <div className="flex items-center">
              ...
              <CopyToClipboard
                text={scimBaseUrl}
                onCopy={() => NotificationsManager.success(t("admin.scim.urlCopied"))}
              >
          ...
          <div>
            <div className="flex items-center mb-2">
              ...
              <Title className="text-lg flex items-center">
                ...
                {t("admin.scim.authToken")}
              </Title>
            </div>

            <Callout title={t("admin.scim.using")} color="blue" className="mb-4">
              {t("admin.scim.usingDescription")}
            </Callout>
            ...
            {!tokenData ? (
              <div className="bg-gray-50 p-4 rounded-lg">
                <Form form={form} onFinish={handleCreateSCIMToken} layout="vertical">
                  <Form.Item
                    name="key_alias"
                    label={t("admin.scim.tokenName")}
                    rules={[{ required: true, message: t("admin.scim.tokenRequired") }]}
                  >
                    <TextInput placeholder={t("admin.scim.tokenPlaceholder")} />
                  </Form.Item>
                  <Form.Item>
                    <TremorButton
                      variant="primary"
              ...
              <Card className="border border-yellow-300 bg-yellow-50">
                ...
                <div className="flex items-center">
                  ...
                  <CopyToClipboard
                    text={tokenData.key}
                    onCopy={() => NotificationsManager.success(t("admin.scim.tokenCopied"))}
                  >
