...
const MCPServerSelector: React.FC<MCPServerSelectorProps> = ({
  ...
  const options = [
    ...
    ...mcpServers.map((server) => ({
      ...
      description: t("virtualKeys.edit.mcpServer"),
    })),
    ...toolsets.map((toolset) => ({
      label: toolset.toolset_name,
      value: `${TOOLSET_PREFIX}${toolset.toolset_id}`,
      description: t("virtualKeys.edit.toolset"),
    })),
  ];
  ...
  const selectOptions: MultiSelectOption[] = [
    ...(allowAllProxyMcpServers || hasAllProxyMcpServersSelected
      ? [{ label: t("virtualKeys.edit.allMcpServers"), value: ALL_PROXY_MCP_SERVERS_SENTINEL }]
      : []),
    ...(allowNoMcpServers
      ? [
          {
            label: t("virtualKeys.edit.blockMcpServers"),
            value: NO_MCP_SERVERS_SENTINEL,
            description: t("virtualKeys.edit.blockAll"),
          },
        ]
      : []),
    ...options.map((opt) => ({
  ...
  return (
    <div>
      <MultiSelect
        ...
        placeholder={placeholder ?? t("virtualKeys.edit.selectMcpServersAndGroups")}
        emptyText={t("virtualKeys.edit.noMcpServers")}
        loading={loading}
        disabled={disabled}
