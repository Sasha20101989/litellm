  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex flex-wrap items-center justify-end gap-4">
        <span className="text-sm text-muted-foreground">{t("virtualKeys.savings.utcDay")}</span>
        <AdvancedDatePicker value={dateValue} onValueChange={onDateChange} />
      </div>

      {!readsWholeKey && (
        <p className="text-sm text-muted-foreground" data-testid="key-savings-scope-note">
          {t("virtualKeys.savings.scopedSpend")}
        </p>
      )}
        ...
        <CardContent>
          ...
          {!hasRows && (
            <p className="py-12 text-center text-sm text-muted-foreground" data-testid="key-savings-empty">
              {isLoading ? t("virtualKeys.savings.loading") : t("virtualKeys.savings.empty")}
            </p>
          )}
