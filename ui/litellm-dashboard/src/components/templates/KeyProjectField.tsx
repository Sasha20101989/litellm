...
export function KeyProjectField({ projectId, canDetach, pending, disabled, onToggle }: KeyProjectFieldProps) {
  ...
  return (
    <Field>
      <FieldLabel htmlFor={id}>{t("virtualKeys.edit.project")}</FieldLabel>
      <Input id={id} value={display ?? ""} disabled readOnly />
      {canDetach && (
        <>
          {pending && <p className="text-sm text-muted-foreground">{t("virtualKeys.edit.projectDetachHelp")}</p>}
          <Button type="button" variant="outline" disabled={disabled} onClick={onToggle}>
            {pending ? t("virtualKeys.edit.keepProject") : t("virtualKeys.edit.detachProject")}
          </Button>
        </>
      )}
    </Field>
