...
const byokSaveErrorMessage = (e: unknown, fallback: string): string => {
...
export const ByokCredentialModal: React.FC<ByokCredentialModalProps> = ({ server, open, onClose, onSuccess }) => {
  ...
  const serverDisplayName = server.alias || server.server_name || t("mcpServers.byokCredentialModal.service");
  ...
  const handleAuthorize = async () => {
    if (!apiKey.trim()) {
      toast.error(t("mcpServers.byokCredentialModal.apiKeyRequired"));
      return;
    }
    setLoading(true);
    try {
      ...
      toast.success(t("mcpServers.byokCredentialModal.connected", { server: serverDisplayName }));
      onSuccess(server.server_id);
      handleClose();
    } catch (e) {
      toast.error(byokSaveErrorMessage(e, t("mcpServers.byokCredentialModal.connectFailed")));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="relative p-2">
          ...
          <div className="flex items-center justify-between mb-6">
            {step === 2 ? (
              ...
              >
                <ArrowLeft className="size-3.5" /> {t("mcpServers.byokCredentialModal.back")}
              </button>
            ) : (
              <div />
            )}
          ...
          {step === 1 ? (
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-3 mb-6">
                <div className="flex size-14 items-center justify-center rounded-xl bg-muted p-1.5 shadow-sm">
                  <img src={nexoplaneLogo.src} alt="Nexoplane" className="size-full rounded-lg object-contain" />
                </div>
              ...
              <h2 className="text-2xl font-bold text-foreground mb-2">
                {t("mcpServers.byokCredentialModal.connect", { server: serverDisplayName })}
              </h2>
              <p className="text-muted-foreground mb-6">
                {t("mcpServers.byokCredentialModal.accessNeeded", { server: serverDisplayName })}
              </p>
              ...
              <div className="bg-muted rounded-xl p-4 text-left mb-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-foreground mb-1">{t("mcpServers.byokCredentialModal.howItWorks")}</p>
                    <p className="text-muted-foreground text-sm">
                      {t("mcpServers.byokCredentialModal.bridge", { server: serverDisplayName })}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {server.byok_description && server.byok_description.length > 0 && (
                <div className="bg-muted rounded-xl p-4 text-left mb-6">
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-3 flex items-center gap-2">
                    ...
                    {t("mcpServers.byokCredentialModal.requestedAccess")}
                  </p>
              ...
              >
                {t("mcpServers.byokCredentialModal.continue")} <ArrowRight className="size-4" />
              </button>
              <button
                onClick={handleClose}
                className="mt-3 w-full text-muted-foreground hover:text-foreground text-sm py-2"
              >
                {t("mcpServers.byokCredentialModal.cancel")}
              </button>
            </div>
          ) : (
            <div>
            ...
            <h2 className="text-2xl font-bold text-foreground mb-2">{t("mcpServers.byokCredentialModal.provideApiKey")}</h2>
              <p className="text-muted-foreground mb-6">
                {t("mcpServers.byokCredentialModal.authorizeConnection", { server: serverDisplayName })}
              </p>

              <div className="mb-4">
                <label htmlFor={apiKeyInputId} className="block text-sm font-semibold text-foreground mb-2">
                  {t("mcpServers.byokCredentialModal.apiKeyLabel", { server: serverDisplayName })}
                </label>
                <PasswordInput
                  id={apiKeyInputId}
                  placeholder={t("mcpServers.byokCredentialModal.apiKeyPlaceholder")}
                  value={apiKey}
                ...
                {server.byok_api_key_help_url && (
                  ...
                  >
                    {t("mcpServers.byokCredentialModal.apiKeyHelp")} <Link2 className="size-3.5" />
                  </a>
                )}
              </div>
              ...
              <div className="bg-muted rounded-xl p-4 flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-foreground">{t("mcpServers.byokCredentialModal.saveKey")}</span>
                </div>
                <Switch checked={saveKey} onCheckedChange={setSaveKey} aria-label={t("mcpServers.byokCredentialModal.saveKey")} />
              </div>
              ...
              <div className="bg-info/10 rounded-xl p-4 flex items-start gap-3 mb-6">
                ...
                <p className="text-sm text-info">
                  {t("mcpServers.byokCredentialModal.securityNote")}
                </p>
              </div>
              ...
              >
                <Lock className="size-4" /> {t("mcpServers.byokCredentialModal.connectAuthorize")}
              </button>
            </div>
          )}
        </div>
