          ...
          <TableCell>
            {value.stored_in_db == true ? (
              <Badge icon={CheckCircleIcon} className="text-white">
                {t("logging.alerts.dynamic.database")}
              </Badge>
            ) : value.stored_in_db == false ? (
              <Badge className="text-gray bg-white outline-solid">{t("logging.alerts.dynamic.config")}</Badge>
            ) : (
              <Badge className="text-gray bg-white outline-solid">{t("logging.alerts.dynamic.unset")}</Badge>
            )}
          </TableCell>
          <TableCell>
            <Icon icon={TrashIcon} color="red" onClick={() => handleResetField(value.field_name, index)}>
              {t("logging.alerts.dynamic.reset")}
            </Icon>
          </TableCell>
        </TableRow>
      ))}
      <div>
        <Button2 htmlType="submit">{t("logging.alerts.dynamic.update")}</Button2>
      </div>
    </Form>
  );
};
