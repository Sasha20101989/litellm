    ...
    <InputGroup>
      ...
      <InputGroupAddon align="inline-end">
        <InputGroupButton
          ...
          aria-label={revealed ? t("virtualKeys.edit.hidePassword") : t("virtualKeys.edit.showPassword")}
        >
          {revealed ? <EyeOff /> : <Eye />}
        </InputGroupButton>
      </InputGroupAddon>
...
const LoggingSettings: React.FC<LoggingSettingsProps> = ({
  ...
  const callbackTypeItems = [
    { value: "success", label: t("virtualKeys.edit.successOnly") },
    { value: "failure", label: t("virtualKeys.edit.failureOnly") },
    { value: "success_and_failure", label: t("virtualKeys.edit.successAndFailure") },
  ];
    ...
    return (
      <div className="mt-6 pt-4 border-t border-border">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <span className="text-sm font-medium text-foreground">{t("virtualKeys.edit.integrationParameters")}</span>
        </div>
          ...
          {Object.entries(dynamicParams).map(([paramName, paramType]) => (
            <div key={paramName} className="space-y-2">
              <label className="text-sm font-medium text-foreground capitalize flex items-center space-x-1">
                <span>{paramName}</span>
                {paramType === "password" && <Badge variant="secondary">{t("virtualKeys.edit.sensitive")}</Badge>}
                {paramType === "number" && <Badge variant="secondary">{t("virtualKeys.edit.number")}</Badge>}
              </label>
              {paramType === "number" && (
                <span className="text-xs text-muted-foreground">{t("virtualKeys.edit.zeroToOne")}</span>
              )}
              {paramType === "number" ? (
                <NumericalInput
                  step={0.01}
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-base font-semibold text-foreground">{t("virtualKeys.edit.disabledCallbacks")}</span>
          <SimpleTooltip content={t("virtualKeys.edit.disabledCallbacksHint")}>
            <Info className="size-4 text-muted-foreground cursor-help" />
          </SimpleTooltip>
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium text-foreground">{t("virtualKeys.edit.disabledCallbacks")}</label>
          <Select multiple value={disabledCallbacks} onValueChange={handleDisabledCallbacksChange}>
            <SelectTrigger className="w-full">
              <SelectValue placeholder={t("virtualKeys.edit.selectDisabledCallbacks")}>
                {(selected: string[]) =>
                  selected.length === 0 ? t("virtualKeys.edit.selectDisabledCallbacks") : selected.join(", ")
                }
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
              {allCallbacks.map((callbackName) => {
                const description = i18n.language?.startsWith("ru")
                  ? t("virtualKeys.edit.loggingIntegrationDescription", { name: callbackName })
          ...
          <div className="text-xs text-muted-foreground">{t("virtualKeys.edit.disabledCallbacksHint")}</div>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-base font-semibold text-foreground">{t("virtualKeys.edit.loggingIntegrations")}</span>
          <SimpleTooltip content={t("virtualKeys.edit.loggingIntegrationsHint")}>
            <Info className="size-4 text-muted-foreground cursor-help" />
          </SimpleTooltip>
        </div>
        <Button variant="secondary" onClick={addLoggingConfig} size="sm" type="button">
          <Plus />
          {t("virtualKeys.edit.addIntegration")}
        </Button>
      </div>
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium">
                    {t("virtualKeys.edit.integrationConfiguration", {
                      name: callbackDisplayName || t("virtualKeys.edit.newIntegration"),
                    })}
                  </span>
                </div>
                <Button
                ...
                >
                  <Trash2 />
                  {t("virtualKeys.edit.removeIntegration")}
                </Button>
              </div>
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground">
                      {t("virtualKeys.edit.integrationType")}
                    </label>
                    <Select
                    ...
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder={t("virtualKeys.edit.selectIntegration")} />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground">{t("virtualKeys.edit.eventType")}</label>
                    <Select
                    ...
                    >
                      <SelectTrigger aria-label={t("virtualKeys.edit.eventType")} className="w-full">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
      ...
      {value.length === 0 && (
        <div className="text-center py-12 text-muted-foreground border-2 border-dashed border-border rounded-lg bg-muted/30">
          ...
          <div className="text-base font-medium mb-1">{t("virtualKeys.edit.noLoggingIntegrations")}</div>
          <div className="text-sm text-muted-foreground">{t("virtualKeys.edit.addIntegrationHint")}</div>
        </div>
      )}
    </div>
  );
