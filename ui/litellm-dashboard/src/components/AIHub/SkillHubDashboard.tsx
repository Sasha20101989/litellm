...
const SkillHubDashboard: React.FC<SkillHubDashboardProps> = ({
  ...
  const domainItems = useMemo(
    () => [{ value: ALL_DOMAINS, label: t("publicHub.skillsDashboard.allDomains") }, ...domains.map((d) => ({ value: d, label: d }))],
    [domains, t],
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="border border-border rounded-lg p-4">
          <div className="text-xs text-muted-foreground mb-1">{t("publicHub.skillsDashboard.totalSkills")}</div>
          <div className="text-2xl font-semibold text-foreground">{totalSkills}</div>
        </div>
        <div className="border border-border rounded-lg p-4">
          <div className="text-xs text-muted-foreground mb-1">{t("publicHub.skillsDashboard.namespaces")}</div>
          <div className="text-2xl font-semibold text-foreground">{namespaces.length}</div>
        </div>
        <div className="border border-border rounded-lg p-4">
          <div className="text-xs text-muted-foreground mb-1">{t("publicHub.skillsDashboard.domains")}</div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-foreground">
            {publicPage ? t("publicHub.skillsDashboard.allPublicSkills") : t("publicHub.skillsDashboard.allSkills")}
          </h3>
        ...
        <div className="mt-3 text-center">
          <p className="text-sm text-muted-foreground">
            {t("publicHub.skillsDashboard.skillsCount", { shown: filteredSkills.length, total: totalSkills })}
          </p>
        </div>
      </div>
    </div>
