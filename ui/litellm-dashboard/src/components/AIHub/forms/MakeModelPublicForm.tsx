...
const MakeModelPublicForm: React.FC<MakeModelPublicFormProps> = ({
  ...
  const handleNext = () => {
    if (currentStep === 0) {
      if (selectedModels.size === 0) {
        NotificationsManager.fromBackend(t("publicHub.forms.models.required"));
        return;
      }
      setCurrentStep(1);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("publicHub.forms.models.success", { count: modelGroupsToMakePublic.length }));
      handleClose();
      onSuccess();
    } catch (error) {
      console.error("Error making model groups public:", error);
      NotificationsManager.fromBackend(t("publicHub.forms.models.failure"));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <Title>{t("publicHub.forms.models.selectTitle")}</Title>
          <div className="flex items-center space-x-2">
            ...
            >
              {t("publicHub.forms.selectAll", { count: filteredData.length })}
            </Checkbox>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {filteredData.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <Text>{t("publicHub.forms.models.empty")}</Text>
              </div>
            ) : (
        ...
        {selectedModels.size > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <Text className="text-sm text-blue-800">
              {t("publicHub.forms.models.selectedCount", { count: selectedModels.size })}
            </Text>
          </div>
        )}
      </div>
  ...
  const renderStep2Content = () => {
    return (
      <div className="space-y-4">
        <Title>{t("publicHub.forms.models.confirmTitle")}</Title>
        ...
        <div className="space-y-3">
          <Text className="font-medium">{t("publicHub.forms.models.selected")}</Text>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <Text className="text-sm text-blue-800">
            {t("publicHub.forms.models.total", { count: selectedModels.size })}
          </Text>
        </div>
      </div>
    );
  ...
  return (
    <Modal
      title={t("publicHub.forms.models.modalTitle")}
      open={visible}
    ...
    >
      <Form form={form} layout="vertical">
        <Steps current={currentStep} className="mb-6">
          <Step title={t("publicHub.forms.models.selectStep")} />
