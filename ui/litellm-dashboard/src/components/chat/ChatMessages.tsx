...
function AssistantBubble({ message, allowImages, isLastMessage, isStreaming, isTypingIndicator, mcpEvents }: AssistantBubbleProps) {
  const { t } = useTranslation("chat");
  const markdownComponentsForMessage: Components = allowImages
    ? markdownComponents
    : {
        ...markdownComponents,
        img: ({ alt }) => <span>{alt || t("messages.imageOmitted")}</span>,
      };
    ...
    <Bubble variant="ghost" className="w-full items-start">
      ...
      <BubbleContent className="w-full text-foreground">
        <ReactMarkdown
          ...
          components={markdownComponentsForMessage}
        >
          {mainContent}
        </ReactMarkdown>
