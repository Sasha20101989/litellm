...
const LatencyBasedConfiguration: React.FC<LatencyBasedConfigurationProps> = ({ routingStrategyArgs }) => {
  ...
  const paramExplanation: { [key: string]: string } = {
    ttl: t("router.latency.ttl"),
    lowest_latency_buffer: t("router.latency.lowestLatencyBuffer"),
  };
    ...
    <>
      <div className="space-y-6">
        <div className="max-w-3xl">
          <h3 className="text-sm font-medium text-gray-900">{t("router.latency.title")}</h3>
          <p className="text-xs text-gray-500 mt-1">{t("router.latency.description")}</p>
        </div>
