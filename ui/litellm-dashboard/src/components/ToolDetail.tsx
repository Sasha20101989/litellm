...
export function ToolDetail({ toolName, onBack, accessToken }: ToolDetailProps) {
  ...
  const handleInputPolicyChange = useCallback(
    async (_name: string, newPolicy: string) => {
      ...
      try {
        ...
        alert(t("toolPolicies.errors.input", { error: e instanceof Error ? e.message : String(e) }));
      } finally {
  ...
  const handleOutputPolicyChange = useCallback(
    async (_name: string, newPolicy: string) => {
      ...
      try {
        ...
        alert(t("toolPolicies.errors.output", { error: e instanceof Error ? e.message : String(e) }));
      } finally {
  ...
  const handleAddOverride = useCallback(async () => {
    ...
    try {
      ...
      alert(t("toolPolicies.errors.addOverride", { error: e instanceof Error ? e.message : String(e) }));
    } finally {
  ...
  const handleRemoveOverride = useCallback(
    async (override: ToolPolicyOverrideRow) => {
      ...
      try {
        ...
        alert(t("toolPolicies.errors.removeOverride", { error: e instanceof Error ? e.message : String(e) }));
      } finally {
        setOverrideSaving(false);
      }
    },
  ...
  if (detailError && !detail) {
    return (
      <div>
        ...
        <p className="text-destructive">{t("toolPolicies.errors.detail")}</p>
      </div>
    );
  }
          ...
          <div>
            <div className="mb-1 flex flex-wrap items-center gap-3">
              ...
              <Badge variant="secondary">
                {t("toolPolicies.detail.calls", { count: (tool.call_count ?? 0).toLocaleString() })}
              </Badge>
            </div>
            <dl className="mt-3 flex flex-wrap gap-x-6 gap-y-1 text-sm text-muted-foreground">
              ...
              {tool.created_at && (
                <div className="flex items-center gap-1.5">
                  ...
                  <dd>{new Date(tool.created_at).toLocaleString(i18n.language === "ru" ? "ru-RU" : "en-US")}</dd>
                </div>
              )}
              {tool.last_used_at && (
                <div className="flex items-center gap-1.5">
                  <dt className="font-medium whitespace-nowrap">{t("toolPolicies.detail.lastUsed")}</dt>
                  <dd>{new Date(tool.last_used_at).toLocaleString(i18n.language === "ru" ? "ru-RU" : "en-US")}</dd>
                </div>
              )}
            </dl>
          </div>
