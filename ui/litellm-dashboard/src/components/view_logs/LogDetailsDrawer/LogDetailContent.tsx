...
const RESPONSE_CACHE_TOOLTIP =
  "Whether this request was served from Nexoplane's response cache (e.g. Redis / in-memory), skipping the LLM provider call entirely. This is separate from provider prompt caching; a Miss here does not mean prompt caching failed.";
const RESPONSE_CACHE_DOCS_URL = "https://docs.litellm.ai/docs/proxy/caching";
const CACHE_KEY_TOOLTIP =
  "The key Nexoplane computed for this request in the response cache. Requests with the same cache key share a cached response; a different key means the request content did not match any cached entry.";
        ...
        <CardContent>
          <DescriptionList>
            ...
            {metadata?.litellm_overhead_time_ms !== undefined && metadata.litellm_overhead_time_ms !== null && (
              <DescriptionItem label="Nexoplane Overhead">
