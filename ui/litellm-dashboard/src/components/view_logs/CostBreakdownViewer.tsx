        ...
        items={[
          {
            ...
            label: (
              <div className="flex items-center justify-between w-full">
                <h3 className="text-lg font-medium text-gray-900">{t("costBreakdown.title")}</h3>
                <div className="flex items-center space-x-2 mr-4">
                  <span className="text-sm text-gray-500">{t("costBreakdown.total")}:</span>
                  <span className="text-sm font-semibold text-gray-900">
                    {formatCost(totalSpend)}
                    {isCached && ` (${t("costBreakdown.cached")})`}
                  </span>
                </div>
              </div>
            ),
            children: (
              <div className="p-6 space-y-4">
                ...
                <div className="space-y-2 max-w-2xl">
                      ...
                      return (
                        <>
                          <div className="flex text-sm">
                            <span className="text-gray-600 font-medium w-1/3">{t("costBreakdown.inputCost")}:</span>
                            <span className="text-gray-900">
                              {formatCost(rawCost)}
                              {rawInputTokens !== undefined && rawInputTokens !== null && (
                                <span className="text-gray-500 font-normal ml-1">
                                  ({rawInputTokens.toLocaleString()} {t("costBreakdown.tokens")})
                                </span>
                              )}
                            </span>
                          </div>
                          {(costBreakdown?.cache_read_cost ?? 0) > 0 && (
                            <div className="flex text-sm">
                              <span className="text-gray-600 font-medium w-1/3">
                                {t("costBreakdown.promptCacheReadCost")}:
                              </span>
                              <span className="text-gray-900">
                                {formatCost(isCached ? 0 : costBreakdown?.cache_read_cost)}
                                {(cacheReadTokens ?? 0) > 0 && (
                                  <span className="text-gray-500 font-normal ml-1">
                                    ({(cacheReadTokens ?? 0).toLocaleString()} {t("costBreakdown.tokens")})
                                  </span>
                                )}
                              </span>
                            </div>
                          ...
                          {(costBreakdown?.cache_creation_cost ?? 0) > 0 && (
                            <div className="flex text-sm">
                              <span className="text-gray-600 font-medium w-1/3">
                                {t("costBreakdown.promptCacheWriteCost")}:
                              </span>
                              <span className="text-gray-900">
                                {formatCost(isCached ? 0 : costBreakdown?.cache_creation_cost)}
                                {(cacheCreationTokens ?? 0) > 0 && (
                                  <span className="text-gray-500 font-normal ml-1">
                                    ({(cacheCreationTokens ?? 0).toLocaleString()} {t("costBreakdown.tokens")})
                                  </span>
                                )}
                              </span>
                            </div>
                      ...
                      <div className="flex text-sm">
                        ...
                        <span className="text-gray-900">
                          ...
                          {promptTokens !== undefined && (
                            <span className="text-gray-500 font-normal ml-1">
                              ({promptTokens.toLocaleString()} {t("costBreakdown.promptTokens")})
                            </span>
                          )}
                        </span>
                      </div>
                  ...
                  <div className="flex text-sm">
                    <span className="text-gray-600 font-medium w-1/3">{t("costBreakdown.outputCost")}:</span>
                    <span className="text-gray-900">
                      {formatCost(outputCost)}
                      {completionTokens !== undefined && (
                        <span className="text-gray-500 font-normal ml-1">
                          ({completionTokens.toLocaleString()} {t("costBreakdown.completionTokens")})
                        </span>
                      )}
                    </span>
                  </div>
                  {costBreakdown?.tool_usage_cost !== undefined && costBreakdown.tool_usage_cost > 0 && (
                    <div className="flex text-sm">
                      <span className="text-gray-600 font-medium w-1/3">{t("costBreakdown.toolUsageCost")}:</span>
                ...
                {!isCached && (
                  <div className="pt-2 border-t border-gray-100 max-w-2xl">
                    <div className="flex text-sm font-semibold">
                      <span className="text-gray-900 w-1/3">{t("costBreakdown.originalLlmCost")}:</span>
                      ...
                      <div className="space-y-2">
                        {costBreakdown.discount_percent !== undefined && costBreakdown.discount_percent !== 0 && (
                          <div className="flex text-sm text-gray-600">
                            <span className="font-medium w-1/3">
                              {t("costBreakdown.discount")} ({formatPercent(costBreakdown.discount_percent)}):
                            </span>
                        ...
                        {costBreakdown.discount_amount !== undefined &&
                          costBreakdown.discount_percent === undefined && (
                            <div className="flex text-sm text-gray-600">
                              <span className="font-medium w-1/3">{t("costBreakdown.discountAmount")}:</span>
                              <span className="text-gray-900">-{formatCost(costBreakdown.discount_amount)}</span>
                            </div>
                          )}
                      </div>
                    ...
                    {hasMargin && (
                      <div className="space-y-2">
                        {costBreakdown.margin_percent !== undefined && costBreakdown.margin_percent !== 0 && (
                          <div className="flex text-sm text-gray-600">
                            <span className="font-medium w-1/3">
                              {t("costBreakdown.margin")} ({formatPercent(costBreakdown.margin_percent)}):
                            </span>
                            <span className="text-gray-900">
                              +
                              {formatCost(
                        ...
                        {costBreakdown.margin_fixed_amount !== undefined && costBreakdown.margin_fixed_amount !== 0 && (
                          <div className="flex text-sm text-gray-600">
                            <span className="font-medium w-1/3">{t("costBreakdown.margin")}:</span>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 max-w-2xl">
                  <div className="flex items-center">
                    <span className="font-bold text-sm text-gray-900 w-1/3">{t("costBreakdown.finalCost")}:</span>
