...
export function VectorStoreViewer({ data }: VectorStoreViewerProps) {
  ...
  const formatTime = (timestamp: number): string => {
    ...
    return date.toLocaleString(i18n.resolvedLanguage === "ru" ? "ru-RU" : "en-US");
  };
    ...
    <div className="bg-white rounded-lg shadow-sm w-full max-w-full overflow-hidden mb-6">
      <Collapse
        ...
        items={[
          {
            key: "1",
            label: <h3 className="text-lg font-medium text-gray-900">{t("vectorStore.title")}</h3>,
            children: (
              <div className="p-4">
                {data.map((request, index) => (
                  <div key={index} className="mb-6 last:mb-0">
                    <div className="bg-white rounded-lg border p-4 mb-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <div className="flex">
                            <span className="font-medium w-1/3">{t("vectorStore.query")}:</span>
                            <span className="font-mono">{request.query}</span>
                          </div>
                          <div className="flex">
                            <span className="font-medium w-1/3">{t("vectorStore.id")}:</span>
                            <span className="font-mono">{request.vector_store_id}</span>
                          </div>
                          <div className="flex">
                            <span className="font-medium w-1/3">{t("vectorStore.provider")}:</span>
                                ...
                                return (
                                  <>
                                    {logo && (
                                      <img
                                        src={logo}
                                        alt={t("vectorStore.logoAlt", { provider: displayName })}
                        ...
                        <div className="space-y-2">
                          <div className="flex">
                            <span className="font-medium w-1/3">{t("vectorStore.startTime")}:</span>
                            <span>{formatTime(request.start_time)}</span>
                          </div>
                          <div className="flex">
                            <span className="font-medium w-1/3">{t("vectorStore.endTime")}:</span>
                            <span>{formatTime(request.end_time)}</span>
                          </div>
                          <div className="flex">
                            <span className="font-medium w-1/3">{t("vectorStore.duration")}:</span>
                    ...
                    <h4 className="font-medium mb-2">{t("vectorStore.results")}</h4>
                    <div className="space-y-2">
                            ...
                            >
                              ...
                              <div className="flex items-center">
                                <span className="font-medium mr-2">
                                  {t("vectorStore.result", { number: resultIndex + 1 })}
                                </span>
                                <span className="text-gray-500 text-sm">
                                  {t("vectorStore.score")}: <span className="font-mono">{result.score.toFixed(4)}</span>
                                </span>
                              </div>
                            </div>
