...
const NotSet = () => {
  ...
  return <span className="italic text-muted-foreground">{t("teams.defaultSettings.notSet")}</span>;
};
...
const TeamSSOSettings: React.FC<TeamSSOSettingsProps> = ({ accessToken }) => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      try {
        ...
        setFetchError(true);
      toast.fromError(t("teams.defaultSettings.fetchFailed"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsEditing(false);
      toast.success(t("teams.defaultSettings.updated"));
    } catch (error) {
      console.error("Error updating team settings:", error);
      toast.fromError(t("teams.defaultSettings.updateFailed"));
    } finally {
      setSaving(false);
    }
  };
  ...
  if (fetchError) {
    return (
      <Card>
        <CardContent>
          <p>{t("teams.defaultSettings.unavailable")}</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="gap-0">
      <CardHeader className="gap-4 border-b border-border pb-6">
        <div>
          <CardTitle>
            <h3 className="text-lg font-semibold text-foreground">{t("teams.defaultSettings.title")}</h3>
          </CardTitle>
          <CardDescription className="mt-1">
            {t("teams.defaultSettings.description")}
          </CardDescription>
        </div>
        <CardAction>
          {isEditing ? (
              ...
              editContent={
                <InputGroup className="max-w-80">
                  ...
                  <InputGroupInput
                    ...
                    aria-label={t("teams.defaultSettings.maxBudget")}
                  />
                </InputGroup>
              }
            />
            ...
            <SettingRow
              ...
              editContent={
                <Input
                  ...
                  placeholder={t("teams.defaultSettings.notSet")}
                  min={0}
                  aria-label={t("teams.defaultSettings.tpmLimit")}
                />
              }
            />
            ...
            <SettingRow
              ...
              editContent={
                <Input
                  ...
                  placeholder={t("teams.defaultSettings.notSet")}
                  min={0}
                  aria-label={t("teams.defaultSettings.rpmLimit")}
                />
              }
            />
          </div>
              ...
              editContent={
                ...
                >
                  ...
                  <ComboboxChipsInput
                    ...
                    aria-label={t("teams.defaultSettings.memberPermissions")}
                  />
                  </ComboboxChips>
