...
export function MCPServerPermissions({
  ...
  const mergedItems = [
    ...directServerIds.map((server) => ({
      type: "server",
      value: server,
      tooltip: t("virtualKeys.sharedDetails.fullId", { id: server }),
    })),
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-foreground">{t("virtualKeys.sharedDetails.mcpServers")}</p>
        <Badge variant={blocksAllMcpServers ? "destructive" : "secondary"}>
          {blocksAllMcpServers
            ? t("virtualKeys.sharedDetails.blocked")
            : grantsAllProxyMcpServers
              ? t("virtualKeys.sharedDetails.all")
              : totalCount}
        </Badge>
      </div>
      ...
      {blocksAllMcpServers ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-destructive/10 border border-destructive/20">
          ...
          <p className="text-destructive text-sm">{t("virtualKeys.sharedDetails.mcpBlocked")}</p>
        </div>
      ) : grantsAllProxyMcpServers ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-info/10 border border-info/20">
          <ServerIcon className="h-4 w-4 text-info" />
          <p className="text-info text-sm">{t("virtualKeys.sharedDetails.allMcpServers")}</p>
        </div>
      ) : totalCount > 0 ? (
                    ...
                    {item.type === "server" ? (
                      ...
                      <div className="inline-flex items-center gap-2 min-w-0">
                        ...
                        <span className="ml-1 px-1.5 py-0.5 text-[9px] font-semibold text-success bg-success/10 border border-success/20 rounded-sm uppercase tracking-wide shrink-0">
                          {t("virtualKeys.sharedDetails.group")}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  {hasToolRestrictions && (
                    <div className="flex items-center gap-1 shrink-0 whitespace-nowrap">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {t("virtualKeys.sharedDetails.tool", { count: toolsForServer.length })}
                      </span>
                      {isExpanded ? (
                  ...
                  >
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <span className="ml-1 px-1.5 py-0.5 text-[9px] font-semibold text-purple-600 bg-purple-50 border border-purple-200 rounded-sm uppercase tracking-wide shrink-0 dark:text-purple-300 dark:bg-purple-950 dark:border-purple-800">
                        {t("virtualKeys.sharedDetails.toolset")}
                      </span>
                    </div>
                    {toolCount > 0 && (
                      <div className="flex items-center gap-1 shrink-0 whitespace-nowrap">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {t("virtualKeys.sharedDetails.tool", { count: toolCount })}
                        </span>
                        {isExpanded ? (
                          <ChevronDownIcon className="h-3.5 w-3.5 text-muted-foreground ml-0.5" />
                        ) : (
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted border border-border">
          ...
          <p className="text-muted-foreground text-sm">{t("virtualKeys.sharedDetails.noMcp")}</p>
        </div>
      )}
    </div>
  );
