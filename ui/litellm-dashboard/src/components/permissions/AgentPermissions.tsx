...
export function AgentPermissions({
  ...
  const mergedItems = [
    ...agents.map((agent) => ({
      type: "agent",
      value: agent,
      tooltip: t("virtualKeys.sharedDetails.fullId", { id: agent }),
    })),
    ...inheritedOnlyAgents.map((grant) => ({
      type: "agent",
      value: grant.id,
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-foreground">{t("virtualKeys.sharedDetails.agents")}</p>
      ...
      {totalCount > 0 ? (
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted border border-border">
          ...
          <p className="text-muted-foreground text-sm">{t("virtualKeys.sharedDetails.noAgents")}</p>
        </div>
      )}
    </div>
  );
