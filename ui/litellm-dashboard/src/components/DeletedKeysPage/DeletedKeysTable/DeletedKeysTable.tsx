...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("deleted.emptyKeysTitle")}</div>
      <div className="text-sm text-muted-foreground">{t("deleted.emptyKeysDescription")}</div>
    </div>
  );
}
...
export function DeletedKeysTable({
  ...
  return (
    <DataTable
      ...
      rowCount={totalCount}
      isLoading={isLoading}
      loadingMessage={t("deleted.loadingKeys")}
