...
const compressionSchema = (t: TFunction<"costOptimization">) => z.object({
  name: z.string().min(1, t("compression.nameRequired")),
  apiBase: z.string().min(1, t("compression.apiBaseRequired")),
  defaultOn: z.boolean(),
});
...
const PromptCompressionTab: React.FC<PromptCompressionTabProps> = ({ accessToken }) => {
  ...
  const schema = useMemo(() => compressionSchema(t), [t]);
  ...
  const loadGuardrails = useCallback(() => {
    ...
    getGuardrailsList(accessToken)
      ...
      .catch((error) => {
        ...
        toast.fromError(t("compression.loadError"));
      })
  ...
  const handleAdd = async (values: CompressionFormValues) => {
    ...
    try {
      ...
      toast.success(t("compression.created"));
      form.reset(EMPTY_VALUES);
      await loadGuardrails();
    } catch (error) {
      console.error("Failed to create compression guardrail:", error);
      toast.fromError(t("compression.createError"));
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <CardContent>
          <TooltipProvider>
            <form onSubmit={form.handleSubmit(handleAdd)} noValidate>
              <FieldGroup>
                <FormField control={form.control} name="name" label={t("compression.name")}>
                ...
                <FormField
                  ...
                  name="apiBase"
                  label={labelWithHint(
                    t("compression.apiBase"),
                    t("compression.apiBaseTooltip"),
                  )}
                  description={t("compression.apiBaseExtra")}
                >
                  {({ ref, ...field }) => <Input {...field} ref={ref} placeholder="https://your-headroom-endpoint" />}
                </FormField>
                <FormField control={form.control} name="defaultOn" label={t("compression.applyAll")}>
