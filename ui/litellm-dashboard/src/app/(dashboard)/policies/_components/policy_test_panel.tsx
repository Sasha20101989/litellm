...
const ContextCombobox: React.FC<ContextComboboxProps> = ({ id, value, onChange, placeholder, options }) => {
  ...
  >
    ...
    <ComboboxContent>
      <ComboboxEmpty>{t("policies.simulator.noOptions")}</ComboboxEmpty>
      <ComboboxList>
        {(item: string) => (
    ...
    <div>
      ...
      {!hasSearched && (
        <div className="bg-card border border-border rounded-lg p-8 text-center">
          ...
          <p className="text-sm font-medium text-foreground mb-1">{t("policies.simulator.empty")}</p>
          <p className="text-xs text-muted-foreground">
            {t("policies.simulator.emptyDescription")}
          </p>
        </div>
      )}
        ...
        <div className="bg-card border border-border rounded-lg p-6">
          {result.matched_policies.length === 0 ? (
            <div className="py-6 text-center">
              ...
              <p className="text-sm text-muted-foreground">{t("policies.simulator.noMatches")}</p>
            </div>
          ) : (
            <>
              <div className="mb-4">
