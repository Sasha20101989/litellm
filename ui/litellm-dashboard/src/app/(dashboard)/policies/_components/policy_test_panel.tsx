    ...
    <div>
      <div className="bg-white border rounded-lg p-6 mb-6">
        <div className="mb-5">
          ...
          <Text type="secondary">{t("policies.simulator.description")}</Text>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Form.Item name="team_alias" label={t("policies.simulator.teamAlias")} className="mb-3">
              <Select
                showSearch
                allowClear
                placeholder={t("policies.simulator.teamPlaceholder")}
            ...
            <Form.Item name="key_alias" label={t("policies.simulator.keyAlias")} className="mb-3">
              <Select
                showSearch
                allowClear
                placeholder={t("policies.simulator.keyPlaceholder")}
      ...
      {!hasSearched && (
        <div className="bg-white border rounded-lg p-8 text-center">
          ...
          <p className="text-sm font-medium text-gray-600 mb-1">{t("policies.simulator.empty")}</p>
          <p className="text-xs text-gray-400">{t("policies.simulator.emptyDescription")}</p>
        </div>
      )}
      ...
      {hasSearched && result && (
        <div className="bg-white border rounded-lg p-6">
          {result.matched_policies.length === 0 ? (
            <Empty description={t("policies.simulator.noMatches")} />
          ) : (
            <>
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {result.effective_guardrails.length > 0 ? (
                    ...
                    <span className="text-gray-400 text-sm">{t("policies.simulator.none")}</span>
                  )}
                </div>
              </div>
                      ...
                      <tr key={p.policy_name} className="border-b last:border-0">
                        ...
                        <td className="py-2">
                          {p.guardrails_added.length > 0 ? (
                            ...
                            <span className="text-gray-400">{t("policies.simulator.none")}</span>
                          )}
                        </td>
                      </tr>
                    ))}
      ...
      {hasSearched && !result && !isLoading && (
        <Alert
          message={t("policies.simulator.error")}
          description={t("policies.simulator.errorDescription")}
          type="error"
          showIcon
        />
      )}
