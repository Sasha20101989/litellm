      ...
      <CardContent className="flex h-full flex-col">
        <div className="mb-4 flex items-start justify-between">
          ...
          <Badge variant="outline">
            {t(`policies.templates.complexity.${complexity.toLowerCase()}`)} {t("policies.templates.complexity.label")}
          </Badge>
        </div>
        ...
        {inherits && (
          <div className="mb-4 text-xs">
            <span className="text-muted-foreground">{t("policies.templates.inherits")} </span>
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        ...
        MessageManager.error(t("policies.templates.fetchFailed"));
      } finally {
        setIsLoading(false);
      }
    };
    ...
    fetchTemplates();
  }, [accessToken, onTemplatesLoaded, t]);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{t("policies.templates.description")}</p>
        </div>
        <Button variant="outline" onClick={onOpenAiSuggestion}>
          ...
          {t("policies.templates.findWithAi")}
        </Button>
      </div>
        ...
        <div className="flex-1">
          {selectedTags.size > 0 && (
            <div className="mb-4 text-sm text-muted-foreground">
              {t("policies.templates.showing", { shown: filteredTemplates.length, total: templates.length })}
            </div>
          )}
