...
const GuardrailSelectionModal: React.FC<GuardrailSelectionModalProps> = ({
  ...
  const summary = (() => {
    if (selectedCount > 0) return <p>{t("policies.guardrailSelection.selected", { count: selectedCount })}</p>;
    if (existingCount > 0) return <p className="text-success">{t("policies.guardrailSelection.allExist")}</p>;
    return <p className="text-warning">{t("policies.guardrailSelection.selectOne")}</p>;
  })();
  const confirmationLabel = (() => {
    if (selectedCount === 0) return t("policies.guardrailSelection.useTemplate");
    if (selectedCount === 1) return t("policies.guardrailSelection.createAndUseOne");
    return t("policies.guardrailSelection.createAndUseMany", { count: selectedCount });
  })();
          ...
          <DialogTitle className="flex items-center gap-2 text-lg">
            ...
            {progressInfo && (
              <Badge variant="secondary">
                {t("policies.guardrailSelection.progress", progressInfo)}
              </Badge>
            )}
          </DialogTitle>
          <DialogDescription>{t("policies.guardrailSelection.description")}</DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4">
          ...
          <div className="mb-4 flex items-center gap-4 rounded-lg border border-border bg-muted p-3">
            ...
            <div className="flex-1">
              <div className="text-sm">
                <span className="font-medium">{t("policies.guardrailSelection.total", { count: guardrailsInfo.length })}</span>
                <span className="mx-2 text-muted-foreground">•</span>
                <span className="font-medium text-success">{t("policies.guardrailSelection.new", { count: newGuardrailsCount })}</span>
                {existingCount > 0 && (
                  <>
                    <span className="mx-2 text-muted-foreground">•</span>
                    <span className="text-muted-foreground">{t("policies.guardrailSelection.existing", { count: existingCount })}</span>
                  </>
                )}
              </div>
            </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {guardrail.alreadyExists && <Badge variant="secondary">{t("policies.guardrailSelection.alreadyExists")}</Badge>}
                    </div>
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      {guardrail.definition?.litellm_params?.patterns && (
                        <Badge variant="secondary">
                          {t("policies.guardrailSelection.patterns", { count: guardrail.definition.litellm_params.patterns.length })}
                        </Badge>
                      )}
                      {guardrail.definition?.litellm_params?.categories && (
                        <Badge variant="secondary">
                          {t("policies.guardrailSelection.categories", { count: guardrail.definition.litellm_params.categories.length })}
                        </Badge>
                      )}
                    </div>
                  </div>
          ...
          {guardrailsInfo.length === 0 && (
            <div className="py-8 text-center text-muted-foreground">
              <p>{t("policies.guardrailSelection.empty")}</p>
              ...
              <div className="rounded-lg border border-border bg-muted p-3">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">
                    {t("policies.guardrailSelection.discoveredCompetitors", { count: template.discoveredCompetitors.length })}
                  </span>
                </div>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {t("policies.guardrailSelection.discoveredDescription")}
                </p>
              </div>
            </>
          )}
