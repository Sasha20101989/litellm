    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-overlay">
      <div className="bg-card rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
        ...
        <h3 className="text-base font-semibold text-foreground mb-1">
          {t(isApprove ? "mcpServers.submissions.confirm.approveTitle" : "mcpServers.submissions.confirm.rejectTitle")}
        </h3>
        <p className="text-sm text-muted-foreground mb-4">
          {t("mcpServers.submissions.confirm.question", {
            action: t(isApprove ? "mcpServers.submissions.confirm.approveAction" : "mcpServers.submissions.confirm.rejectAction"),
            name: serverName,
          })}{" "}
          {isApprove
            ? t("mcpServers.submissions.confirm.approveBody")
            : rejectBody}
        </p>
        {!isApprove && (
          <textarea
        ...
        <div className="flex gap-3">
          ...
          >
            {t(isApprove ? "mcpServers.submissions.confirm.approve" : "mcpServers.submissions.confirm.reject")}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="mb-5 border border-border rounded-lg bg-card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">{t("mcpServers.submissions.rules.title")}</span>
          {activeLabels.length > 0 ? (
            <span className="text-xs text-muted-foreground">
              ({t("mcpServers.submissions.rules.requiredCount", { count: activeLabels.length })})
            </span>
          ) : (
            <span className="text-xs text-muted-foreground italic">{t("mcpServers.submissions.rules.none")}</span>
          )}
        </div>
      ...
      {expanded && (
        <div className="border-t border-border px-4 pt-4 pb-4">
          <p className="text-xs text-muted-foreground mb-4">
            {t("mcpServers.submissions.rules.description")}
          </p>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              >
                ...
                {t(`mcpServers.submissions.statuses.${approvalStatus}`)}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 mt-1.5 text-xs text-muted-foreground">
              <span>
                {t("mcpServers.submissions.card.transport")} <span className="text-muted-foreground">{server.transport ?? "sse"}</span>
              </span>
              <span>·</span>
              <span>
                {t("mcpServers.submissions.card.submittedBy")} <span className="text-muted-foreground">{server.submitted_by ?? "—"}</span>
              </span>
              <span>·</span>
              <span>{formatDate(server.submitted_at)}</span>
            </div>
            {approvalStatus === "rejected" && server.review_notes && (
              <p className="text-xs text-destructive mt-1.5">{t("mcpServers.submissions.card.rejectionReason")} {server.review_notes}</p>
            )}
          </div>
            ...
            {checks.map((c) => (
              <div key={c.key} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <span className={`text-xs ${c.passed ? "text-success" : "text-destructive"}`}>
                  {t(c.passed ? "mcpServers.submissions.card.passes" : "mcpServers.submissions.card.missing")}
                </span>
              </div>
            ))}
          </div>
...
export function MCPSubmissionsTab({ accessToken }: MCPSubmissionsTabProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("mcpServers.submissions.loadFailed"));
    } finally {
      setIsLoading(false);
    }
  }, [accessToken, t]);
  ...
  const handleSaveRules = async () => {
    ...
    try {
      ...
      toast.success(t("mcpServers.submissions.rules.saved"));
    } catch {
      toast.fromError(t("mcpServers.submissions.rules.saveFailed"));
    } finally {
  ...
  async function handleApprove(serverId: string, serverName: string) {
    ...
    try {
      ...
      await fetchData();
      toast.success(t("mcpServers.submissions.approved", { name: serverName }));
    } catch {
      toast.fromError(t("mcpServers.submissions.approveFailed"));
    } finally {
  ...
  async function handleReject(serverId: string, serverName: string, reviewNotes?: string) {
    ...
    try {
      ...
      await fetchData();
      toast.success(t("mcpServers.submissions.rejected", { name: serverName }));
    } catch {
      toast.fromError(t("mcpServers.submissions.rejectFailed"));
    } finally {
      setConfirmAction(null);
    }
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <StatCard label={t("mcpServers.submissions.stats.total")} value={summary.total} color="text-foreground" />
        <StatCard label={t("mcpServers.submissions.stats.pending")} value={summary.pending_review} color="text-warning" />
        <StatCard label={t("mcpServers.submissions.stats.active")} value={summary.active} color="text-success" />
        <StatCard label={t("mcpServers.submissions.stats.rejected")} value={summary.rejected} color="text-destructive" />
      </div>
      ...
      <div className="space-y-3">
        {isLoading && <div className="text-center py-12 text-muted-foreground text-sm">{t("mcpServers.submissions.loading")}</div>}
        {error && <div className="text-center py-12 text-destructive text-sm">{error}</div>}
        {!isLoading && !error && filtered.length === 0 && (
          <div className="text-center py-12 text-muted-foreground text-sm">
            {t("mcpServers.submissions.empty")}
          </div>
        )}
        {!isLoading &&
          !error &&
