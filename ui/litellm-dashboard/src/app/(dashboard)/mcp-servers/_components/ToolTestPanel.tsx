...
export function ToolTestPanel({
  ...
  const handleCopyResult = async () => {
    ...
    if (success) {
      toast.success(t("mcpServers.toolTest.resultCopied"));
    } else {
      toast.fromError(t("mcpServers.toolTest.resultCopyFailed"));
    }
  };
  ...
  const handleCopyToolName = async () => {
    ...
    if (success) {
      toast.success(t("mcpServers.toolTest.nameCopied"));
    } else {
      toast.fromError(t("mcpServers.toolTest.nameCopyFailed"));
    }
  };
  ...
  return (
    <div className="space-y-4 h-full">
        ...
        <div className="flex items-center space-x-3">
          {tool.mcp_info.logo_url && (
            ...
            <img
              ...
              alt={t("mcpServers.toolTest.serverLogo", { name: tool.mcp_info.server_name })}
              className="w-6 h-6 object-contain"
            />
          )}
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2 mb-1">
              <h2 className="text-lg font-semibold text-foreground">{t("mcpServers.toolTest.testTool")}</h2>
              <div
            ...
            <p className="text-xs text-muted-foreground">{t("mcpServers.toolTest.provider", { name: tool.mcp_info.server_name })}</p>
          </div>
        </div>
        <Button
          onClick={onClose}
          variant="ghost"
          size="icon-sm"
          aria-label={t("mcpServers.toolTest.close")}
      ...
      <div className="grid grid-cols-2 gap-4 h-full">
        ...
        <div className="bg-card border border-border rounded-lg">
          <div className="border-b border-border px-4 py-2">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-foreground">{t("mcpServers.toolTest.inputParameters")}</h3>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger
                    render={<CircleHelp className="size-4 cursor-help text-muted-foreground hover:text-foreground" />}
                  />
                  <TooltipContent>{t("mcpServers.toolTest.inputTooltip")}</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          </div>
        ...
        <div className="bg-card border border-border rounded-lg">
          <div className="border-b border-border px-4 py-2">
            <h3 className="text-sm font-semibold text-foreground">{t("mcpServers.toolTest.result")}</h3>
          </div>
            ...
            {!result && !error && !isLoading ? (
              ...
              <div className="flex flex-col justify-center items-center h-48 text-muted-foreground">
                <div className="text-center max-w-sm">
                  ...
                  <h4 className="text-sm font-medium text-foreground mb-1">{t("mcpServers.toolTest.ready")}</h4>
                  ...
                  <div className="p-2 bg-success/10 border border-success/20 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <h4 className="text-xs font-medium text-success">{t("mcpServers.toolTest.success")}</h4>
                        {duration !== null && (
                          <span className="text-xs text-success ml-1">
                            • {t("mcpServers.toolTest.duration", { value: (duration / 1000).toFixed(2) })}
                          </span>
                        )}
                      </div>
                ...
                <div className="max-h-96 overflow-y-auto">
                  {isLoading && (
                    <div className="flex flex-col justify-center items-center h-48 text-muted-foreground">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{t("mcpServers.toolTest.wait")}</p>
                    </div>
                  )}
                    ...
                    <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3">
                      <div className="flex items-start space-x-2">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-1">
                            <h4 className="text-xs font-medium text-destructive">{t("mcpServers.toolTest.failed")}</h4>
                            {duration !== null && (
