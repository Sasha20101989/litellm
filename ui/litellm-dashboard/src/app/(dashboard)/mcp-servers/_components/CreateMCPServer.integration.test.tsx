...
describe("CreateMCPServer", () => {
  ...
  describe("when HTTP transport is selected", () => {
    ...
    it("should warn that LiteLLM auth is disabled when True Passthrough is selected", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("True Passthrough disables Nexoplane authentication for this server"),
        ).toBeInTheDocument();
      });
    });
    ...
    it.each([["True Passthrough (no Nexoplane auth)"], ["OAuth Delegate (client-supplied upstream token)"]])(
    ...
    it.each([
      ["true_passthrough", "True Passthrough (no Nexoplane auth)"],
