...
const MCPNetworkSettings: React.FC<MCPNetworkSettingsProps> = ({ accessToken }) => {
  ...
  return (
    <div className="space-y-6 p-4">
      <DeprecationBanner featureName={t("mcpServers.networkSettings.featureName")} />
      <div>
        <p className="text-lg font-semibold">{t("mcpServers.networkSettings.title")}</p>
        <p className="mt-1 text-sm text-muted-foreground">{t("mcpServers.networkSettings.description")}</p>
      </div>
      ...
      <Card className="p-6">
        {currentIp && (
          <div className="mb-4 rounded-lg bg-muted p-3">
            <p className="text-sm">
              {t("mcpServers.networkSettings.currentIp")} <span className="font-mono font-medium">{currentIp}</span>
            </p>
            {suggestedRange && !privateRanges.includes(suggestedRange) && (
              <div className="mt-1 flex items-center gap-2">
                <p className="text-sm">{t("mcpServers.networkSettings.suggestedRange")} </p>
                <Button
                  variant="outline"
                  size="sm"
                  className="font-mono"
        ...
        <div className="mb-2 flex items-center">
          <p className="text-sm font-medium">{t("mcpServers.networkSettings.ranges")}</p>
        </div>
          ...
          <div className="mb-2 flex flex-wrap gap-1.5">
            {privateRanges.map((range) => (
              <Badge key={range} variant="secondary" className="font-mono">
                ...
                <button
                  type="button"
                  aria-label={t("mcpServers.networkSettings.removeRange", { range })}
        ...
        <Input
          value={rangeDraft}
          placeholder={t("mcpServers.networkSettings.placeholder")}
        ...
        />
        <p className="mt-2 text-xs text-muted-foreground">{t("mcpServers.networkSettings.hint")}</p>
      </Card>

      <div className="flex justify-end">
        <Button onClick={handleSave} disabled={saving}>
          <Save />
          {t("mcpServers.networkSettings.save")}
        </Button>
      </div>
    </div>
  );
