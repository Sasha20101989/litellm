              ...
              <DropdownMenuTrigger
                render={
                  <button
                    ...
                    onClick={stop}
                    onKeyDown={stop}
                    aria-label={t("mcpServers.card.actions")}
              ...
              <TooltipTrigger
                render={
                  <Badge variant="outline">
                    <CircleAlert />
                    {t("mcpServers.card.oauthUnset")}
                  </Badge>
                }
              />
              <TooltipContent>{t("mcpServers.card.oauthUnsetHint")}</TooltipContent>
            </Tooltip>
          )}
          <Badge variant="outline">
                  ...
                  <TooltipTrigger
                    render={
                      <span className="inline-flex items-center gap-1 font-semibold text-destructive">
                        ...
                        {t("mcpServers.card.missingFields", { count: missing.length })}
                      </span>
                    }
                  />
                  <TooltipContent>
                    <div className="mb-1 font-semibold">{t("mcpServers.card.missingFieldsTitle")}</div>
                    <ul className="ml-3">
                      {missing.map((m) => (
                        <li key={m}>• {m}</li>
                      ))}
...
const HealthChip: FC<HealthChipProps> = ({
  ...
  const statusLabel =
    status === "healthy"
      ? t("mcpServers.card.healthy")
      : status === "unhealthy"
        ? t("mcpServers.card.unhealthy")
        : t("mcpServers.card.unknown");
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="top" className="max-w-xs">
        <div className="mb-1 font-semibold">{t("mcpServers.card.health", { status: statusLabel })}</div>
        {lastCheck && (
          <div className="mb-1 text-xs">
            {t("mcpServers.card.lastCheck", { date: new Date(lastCheck).toLocaleString(i18n.language) })}
          </div>
        )}
        {error && (
          <div className="text-xs">
            <div className="mb-1 font-medium">{t("mcpServers.card.error")}</div>
...
const ByokRow: FC<ByokRowProps> = ({ connected, onConnect }) => {
  ...
  if (connected) {
    return (
      <div className="flex items-center justify-between gap-2 text-xs">
        <span className="text-muted-foreground">{t("mcpServers.card.byok")}</span>
