      ...
      existingOptions.push({
        ...
        label: (
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-400 text-xs ml-1">{t("mcpServers.create.createGroup")}</span>
          </div>
        ),
      });
    }
    ...
    try {
        ...
        if (rawStdioConfig) {
          try {
            ...
            if (!stdioFields.command) {
              NotificationsManager.fromBackend(t("mcpServers.create.errors.stdioCommand"));
              return;
            }
          } catch {
            NotificationsManager.fromBackend(t("mcpServers.create.errors.invalidStdio"));
            return;
          }
        } else {
          ...
          if (rawEnvJson) {
            try {
              ...
              NotificationsManager.fromBackend(t("mcpServers.create.errors.invalidStdioEnv"));
              return;
            }
          }
      ...
      if (rawTokenValidationJson && rawTokenValidationJson.trim() !== "") {
        try {
          ...
          NotificationsManager.fromBackend(t("mcpServers.create.errors.invalidValidationRules"));
          return;
        }
      }
      ...
      if (oauthTokenResponse?.access_token) {
        ...
        try {
          ...
          NotificationsManager.fromBackend(
            t("mcpServers.edit.oauthPersistFailed", { reason: message ? `: ${message}` : "" }),
          );
          return;
        }
      }

      NotificationsManager.success(t("mcpServers.edit.updated"));
      setAppMayNotMatchUpstream(false);
      onSuccess(updated);
    } catch (error: any) {
      NotificationsManager.fromBackend(
        t("mcpServers.edit.updateFailed", { reason: error?.message ? `: ${error.message}` : "" }),
      );
    }
  };
  ...
  return (
    <TabGroup>
      <TabList className="grid w-full grid-cols-2">
        <Tab>{t("mcpServers.edit.serverTab")}</Tab>
        <Tab>{t("mcpServers.edit.costTab")}</Tab>
      </TabList>
      <TabPanels className="mt-6">
        <TabPanel>
          ...
          >
            ...
            <Form.Item label={t("mcpServers.create.description")} name="description">
              <Input className="rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" />
            </Form.Item>
            <MCPLogoSelector value={logoUrl} onChange={setLogoUrl} />
            <Form.Item label={t("mcpServers.create.transport")} name="transport" rules={[{ required: true }]}>
              <Select onChange={handleTransportChange}>
                <Select.Option value="http">{t("mcpServers.create.streamableHttp")}</Select.Option>
                <Select.Option value="sse">{t("mcpServers.create.sse")}</Select.Option>
                <Select.Option value="stdio">{t("mcpServers.create.stdio")}</Select.Option>
                <Select.Option value={TRANSPORT.OPENAPI}>{t("mcpServers.create.openapi")}</Select.Option>
              </Select>
            </Form.Item>
            ...
            {isMCPTransport && (
              <Form.Item
                label={t("mcpServers.create.serverUrl")}
                name="url"
                rules={[
                  { required: true, message: t("mcpServers.create.serverUrlRequired") },
              ...
              <Form.Item
                label={
                  <span className="text-sm font-medium text-gray-700 flex items-center">
                    {t("mcpServers.edit.specUrl")}
                    <Tooltip title={t("mcpServers.edit.specUrlHint")}>
                      <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                    </Tooltip>
                  </span>
                }
                name="spec_path"
                rules={[{ required: true, message: t("mcpServers.edit.specUrlRequired") }]}
              >
                <Input
            ...
            <Form.Item
              label={
                <span className="text-sm font-medium text-gray-700 flex items-center">
                  {t("mcpServers.create.concurrency")}
                  <Tooltip title={t("mcpServers.create.concurrencyHint")}>
                    <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                  </Tooltip>
                </span>
              }
            ...
            >
              <InputNumber
                min={1}
                precision={0}
                placeholder={t("mcpServers.create.concurrencyPlaceholder")}
                style={{ width: "100%" }}
            ...
            {!isStdioTransport && (
              <>
                <Form.Item label={t("mcpServers.create.authentication")} name="auth_type" rules={[{ required: true }]}>
                  <Select virtual={false}>
                    <Select.Option value="none">{t("mcpServers.create.authTypes.none")}</Select.Option>
                    <Select.Option value="api_key">{t("mcpServers.create.authTypes.apiKey")}</Select.Option>
                    <Select.Option value="bearer_token">{t("mcpServers.create.authTypes.bearer")}</Select.Option>
            ...
            {isStdioTransport && (
              <div className="rounded-lg border border-gray-200 p-4 space-y-4">
                ...
                <Form.Item
                  ...
                  name="command"
                  rules={[{ required: true, message: t("mcpServers.edit.commandRequired") }]}
                >
                  <Input
                    placeholder={t("mcpServers.edit.commandPlaceholder")}
                    className="rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                  />
                </Form.Item>

                <Form.Item label={t("mcpServers.edit.args")} name="args">
                  <Select
                    mode="tags"
                    size="large"
                    tokenSeparators={[","]}
                    placeholder={t("mcpServers.edit.argsPlaceholder")}
                    className="rounded-lg"
                  />
                </Form.Item>

                <Form.Item
                  label={t("mcpServers.edit.environment")}
                  name="env_json"
                  rules={[
                    {
                      validator: (_, value) => {
                        ...
                        try {
                          ...
                          return Promise.reject(new Error(t("mcpServers.edit.envObject")));
                        } catch {
                          return Promise.reject(new Error(t("mcpServers.edit.validJson")));
                        }
                      },
                    },
                  ]}
            ...
            {!isStdioTransport && shouldShowAuthValueField && (
              <Form.Item
                label={
                  <span className="text-sm font-medium text-gray-700 flex items-center">
                    {t("mcpServers.create.authValue")}
                    <Tooltip title={t("mcpServers.create.authValueHint")}>
                      <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                    </Tooltip>
                  </span>
                }
                ...
                rules={[
                  {
                    validator: (_, value) =>
                      value && typeof value === "string" && value.trim() === ""
                        ? Promise.reject(new Error(t("mcpServers.create.authValueWhitespace")))
                        : Promise.resolve(),
                  },
                ]}
              >
                <Input.Password
                  placeholder={t("mcpServers.edit.authValueExisting")}
                  className="rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                />
              </Form.Item>
            )}
              ...
              <>
                {!oauthFlowTypeValue && !isDelegateAuth && (
                  <Alert
                    ...
                    message={t("mcpServers.edit.oauthUnset")}
                    description={t("mcpServers.edit.oauthUnsetDescription")}
                  />
                )}
                <OAuthFormFields
                  isM2M={isM2MFlow}
            ...
            {!isStdioTransport && isAwsSigV4AuthType && (
              <>
                ...
                >
                  <Input.Password
                    placeholder={t("mcpServers.edit.keepExisting")}
                    className="rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                  />
                </Form.Item>
                <Form.Item
            ...
            <div className="flex justify-end gap-2">
              <AntdButton onClick={onCancel}>{t("mcpServers.edit.cancel")}</AntdButton>
              <Button type="submit">{t("mcpServers.edit.save")}</Button>
            </div>
          </Form>
        </TabPanel>
        ...
        <TabPanel>
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-2">
              <AntdButton onClick={onCancel}>{t("mcpServers.edit.cancel")}</AntdButton>
              <Button onClick={() => form.submit()}>{t("mcpServers.edit.save")}</Button>
            </div>
          </div>
        </TabPanel>
      </TabPanels>
