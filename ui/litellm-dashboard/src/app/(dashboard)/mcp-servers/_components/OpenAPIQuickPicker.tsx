...
const OpenAPIQuickPicker: React.FC<OpenAPIQuickPickerProps> = ({ accessToken, selectedName, onSelect }) => {
  ...
  if (loading) {
    return (
      <div className="mb-4">
        <span className="text-sm font-medium">{t("mcpServers.forms.openapi.popular")}</span>
  ...
  return (
    <div className="mb-4">
      <span className="mb-2 block text-sm font-medium">{t("mcpServers.forms.openapi.popular")}</span>
