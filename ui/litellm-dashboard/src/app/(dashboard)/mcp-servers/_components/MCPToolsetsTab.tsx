        ...
        <span className="text-sm font-medium text-gray-700 flex items-center gap-2">
          ...
          {selectedSet.size > 0 && (
            <span className="ml-1 text-xs text-purple-600 font-semibold">
              {t("mcpServers.toolsets.selectedCount", { count: selectedSet.size })}
            </span>
          )}
        </span>
      ...
      {expanded && (
        <div className="p-2">
          {loading ? (
            ...
            <p className="text-xs text-gray-400 px-2 py-2">{t("mcpServers.toolsets.noToolsForServer")}</p>
          ) : (
...
function CreateToolsetModal({ open, onClose, onSave, accessToken, initialToolset }: CreateToolsetModalProps) {
  ...
  return (
    <Modal
      open={open}
      onCancel={onClose}
      title={initialToolset ? t("mcpServers.toolsets.edit") : t("mcpServers.toolsets.new")}
      width={960}
      footer={null}
      forceRender
    >
      <Form form={form} layout="vertical" className="mt-2">
        <div className="flex gap-4 mb-4">
          <Form.Item
            label={t("mcpServers.toolsets.name")}
            name="toolset_name"
            rules={[{ required: true, message: t("mcpServers.toolsets.nameRequired") }]}
            className="flex-1 mb-0"
          >
            <Input placeholder={t("mcpServers.toolsets.namePlaceholder")} />
          </Form.Item>
          <Form.Item label={t("mcpServers.toolsets.descriptionLabel")} name="description" className="flex-1 mb-0">
            <Input placeholder={t("mcpServers.toolsets.descriptionPlaceholder")} />
          </Form.Item>
        </div>
      </Form>
      ...
      <div className="flex gap-4 mt-2" style={{ minHeight: 360 }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2">
            <Text className="text-sm font-semibold text-gray-700">{t("mcpServers.toolsets.availableTools")}</Text>
          </div>
          <Input
            placeholder={t("mcpServers.toolsets.searchServers")}
            value={serverSearch}
          ...
          <div className="space-y-2 overflow-y-auto" style={{ maxHeight: 300 }}>
            {filteredServers.length === 0 ? (
              <Text className="text-gray-400 text-sm">
                {mcpServers.length === 0
                  ? t("mcpServers.toolsets.noServers")
                  : t("mcpServers.toolsets.noServerMatches")}
              </Text>
            ) : (
        ...
        <div className="w-72 shrink-0">
          <Text className="text-sm font-semibold text-gray-700 mb-2 block">
            {t("mcpServers.toolsets.yourToolset")}{" "}
            <span className="text-xs font-normal text-gray-400">
              ({t("mcpServers.toolsets.toolsCount", { count: selectedTools.length })})
            </span>
          </Text>
          <div className="space-y-1 overflow-y-auto" style={{ maxHeight: 340 }}>
            {selectedTools.length === 0 ? (
              <Text className="text-gray-400 text-sm">{t("mcpServers.toolsets.noToolsAdded")}</Text>
            ) : (
      ...
      <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-gray-200">
        <Button variant="secondary" onClick={onClose}>
          {t("mcpServers.toolsets.cancel")}
        </Button>
        <Button onClick={handleSubmit} loading={saving}>
          {initialToolset ? t("mcpServers.toolsets.saveChanges") : t("mcpServers.toolsets.create")}
        </Button>
      </div>
    </Modal>
  );
...
function ToolsetsEmptyState() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("mcpServers.toolsets.emptyTitle")}</div>
      <div className="text-sm text-muted-foreground">{t("mcpServers.toolsets.emptyDescription")}</div>
    </div>
  );
}
...
function ToolsetUsageGuide() {
  ...
  return (
    <div className="mb-6 rounded-lg border border-gray-200 bg-gray-50 px-5 py-4">
      <p className="text-sm font-medium text-gray-700 mb-1">{t("mcpServers.toolsets.guideTitle")}</p>
      <p className="text-sm text-gray-500 mb-3">
        {t("mcpServers.toolsets.guideBefore")}{" "}
        <span className="font-medium text-gray-700">{t("mcpServers.toolsets.guidePath")}</span>,{" "}
        {t("mcpServers.toolsets.guideAfter")}
      </p>
      <div className="text-xs text-gray-400 mb-1">{t("mcpServers.toolsets.configLabel")}</div>
      <div className="relative">
        ...
        >
          {copied ? "✓" : t("mcpServers.toolsets.copy")}
        </button>
      </div>
    </div>
  );
...
export function MCPToolsetsTab({ accessToken, userRole }: MCPToolsetsTabProps) {
  ...
  const handleCreate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    message.success(t("mcpServers.toolsets.created"));
  ...
  const handleUpdate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    message.success(t("mcpServers.toolsets.updated"));
    queryClient.invalidateQueries({ queryKey: ["mcpToolsets"] });
    setEditToolset(null);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      message.success(t("mcpServers.toolsets.deleted"));
      queryClient.invalidateQueries({ queryKey: ["mcpToolsets"] });
      setDeleteId(null);
    } finally {
      setDeleting(false);
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <Title>{t("mcpServers.toolsets.title")}</Title>
          <Text className="text-gray-500 text-sm">{t("mcpServers.toolsets.description")}</Text>
        </div>
        {isAdmin && (
          <Button icon={PlusIcon} onClick={() => setCreateOpen(true)}>
            {t("mcpServers.toolsets.new")}
          </Button>
        )}
      </div>
      ...
      <DataTable
        ...
        isLoading={isLoading}
        loadingMessage={t("mcpServers.toolsets.loading")}
      ...
      <Modal
        ...
        onOk={handleDelete}
        okText={t("mcpServers.toolsets.delete")}
        okButtonProps={{ danger: true, loading: deleting }}
        title={t("mcpServers.toolsets.deleteTitle")}
      >
        <p>{t("mcpServers.toolsets.deleteWarning")}</p>
      </Modal>
    </div>
  );
}
