      ...
      <div className="space-y-4">
        {hasDefaultCost &&
          ...
          costConfig?.default_cost_per_query !== null && (
            <div>
              <p className="text-sm font-medium">{t("mcpServers.costDisplay.defaultCost")}</p>
        ...
        {hasToolCosts && costConfig?.tool_name_to_cost_per_query && (
          <div>
            <p className="text-sm font-medium">{t("mcpServers.costDisplay.toolCosts")}</p>
                  ...
                  cost !== undefined && (
                    <div key={toolName} className="flex items-center justify-between rounded-lg bg-muted p-3">
                      ...
                      <p className="font-mono text-sm">
                        {t("mcpServers.costDisplay.perQuery", { cost: cost.toFixed(4) })}
                      </p>
                    </div>
                  ),
              )}
        ...
        <div className="mt-4 rounded-lg border border-border bg-muted p-4">
          <p className="text-sm font-medium">{t("mcpServers.costDisplay.summary")}</p>
          <div className="mt-2 space-y-1">
            {hasDefaultCost &&
              costConfig?.default_cost_per_query !== undefined &&
              costConfig?.default_cost_per_query !== null && (
                <p className="text-sm text-muted-foreground">
                  {t("mcpServers.costDisplay.defaultSummary", {
            ...
            {hasToolCosts && costConfig?.tool_name_to_cost_per_query && (
              <p className="text-sm text-muted-foreground">
                {t("mcpServers.costDisplay.customCount", {
