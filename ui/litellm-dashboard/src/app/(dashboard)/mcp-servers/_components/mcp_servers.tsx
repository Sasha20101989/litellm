...
const MCPServers: React.FC<MCPServerProps> = ({ accessToken, userRole, userID }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("mcpServers.delete.success"));
  ...
  if (!accessToken || !userRole || !userID) {
    return <div className="p-6 text-center text-gray-500">{t("mcpServers.missingAuth")}</div>;
  }
        ...
        <AlertDialog open={isDeleteModalOpen} onOpenChange={(open) => !open && cancelDelete()}>
          <AlertDialogContent>
            ...
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">{t("mcpServers.delete.warning")}</p>
              ...
              <div className="w-full h-full">
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="all">
                              {isInternalUser
                                ? t("mcpServers.filters.allAvailable")
                                : t("mcpServers.filters.allServers")}
                            </SelectItem>
                      ...
                      <div className="flex items-center gap-2">
                        <p className="flex items-center text-sm font-medium whitespace-nowrap text-muted-foreground">
                          ...
                          <Tooltip>
                            ...
                            />
                            <TooltipContent>{t("mcpServers.filters.accessGroupHint")}</TooltipContent>
                          </Tooltip>
                        </p>
                        <Select
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  ...
                  <div className="ml-auto text-xs text-muted-foreground">
                    {t("mcpServers.filters.count", {
                      visible: displayedServers.length,
                  ...
                  {isLoadingServers ? (
                    ...
                    <div className="rounded-lg border border-dashed border-border bg-card p-12 text-center">
                      <p className="text-sm text-muted-foreground">
                        {filteredServers.length === 0 ? t("mcpServers.empty") : t("mcpServers.emptyFiltered")}
                      </p>
                    </div>
                  ) : (
                    <div
