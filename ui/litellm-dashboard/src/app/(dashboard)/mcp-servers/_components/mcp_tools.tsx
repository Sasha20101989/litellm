      ...
      <Card className="w-full overflow-hidden rounded-xl shadow-md">
        <div className="grid h-auto w-full grid-cols-4 gap-4">
          ...
          <div className="col-span-1 flex flex-col bg-muted p-4">
            <h2 className="mt-2 mb-6 text-xl font-semibold">{t("mcpServers.toolsViewer.title")}</h2>
            ...
            <div className="flex flex-col flex-1">
                ...
                <div className="mb-4 rounded-lg border border-border bg-card p-3">
                  <div className="mb-2 flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <p className="text-sm font-medium">{t("mcpServers.toolsViewer.additionalHeaders")}</p>
                    </div>
                    <Button variant="link" size="sm" onClick={() => setShowHeaderInput(!showHeaderInput)}>
                      {showHeaderInput ? t("mcpServers.toolsViewer.hide") : t("mcpServers.toolsViewer.configure")}
                    </Button>
                  </div>
                  ...
                  {showHeaderInput && (
                    <div className="mt-3 space-y-2">
                      {extraHeaders?.map((headerName) => (
                        <div key={headerName}>
                          ...
                          <InputGroup className="w-full">
                            ...
                            <InputGroupInput
                              placeholder={t("mcpServers.toolsViewer.enterHeader", { header: headerName })}
                      ...
                      >
                        {t("mcpServers.toolsViewer.loadTools")}
                      </Button>
                    </div>
                  )}
                  ...
                  {!showHeaderInput && Object.keys(passthroughHeaders).length > 0 && (
                    <div className="mt-2">
                      <p className="flex items-center text-xs text-muted-foreground">
                        ...
                        {t("mcpServers.toolsViewer.configuredHeaders", { count: Object.keys(passthroughHeaders).length })}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col flex-1 min-h-0">
                <p className="mb-3 flex items-center text-sm font-medium">
                  <Wrench className="mr-2 size-4" /> {t("mcpServers.toolsViewer.availableTools")}
                  {toolsData.length > 0 && (
                ...
                {usesBrowserHeldToken && !oauthToken && (
                  <div className="rounded-lg border border-border bg-card p-4 text-center">
                    ...
                    <p className="mb-3 text-xs text-muted-foreground">{t("mcpServers.toolsViewer.authToView")}</p>
                    <Button
                      size="sm"
                      onClick={startOAuthFlow}
                      disabled={!accessToken || oauthStatus === "authorizing" || oauthStatus === "exchanging"}
                    >
                      {t("mcpServers.toolsViewer.authorize")}
                    </Button>
                  ...
                  <>
                    {toolsData.length > 0 && (
                      <div className="mb-3">
                        <InputGroup className="w-full">
                          ...
                          <InputGroupInput
                            placeholder={t("mcpServers.toolsViewer.search")}
                    ...
                    {toolsAreaLoading && (
                      <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card py-8">
                        ...
                        <p className="text-xs font-medium">{t("mcpServers.toolsViewer.loading")}</p>
                      </div>
                    )}
                    ...
                    {(mcpToolsResponse?.error || mcpToolsError) && !toolsAreaLoading && !toolsData.length && (
                      <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-xs text-destructive">
                        <p className="font-medium">
                          {t("mcpServers.toolsViewer.error", { message: mcpToolsResponse?.message || (mcpToolsError as Error)?.message })}
                        </p>
                      </div>
                    )}
                    ...
                    {!toolsAreaLoading &&
                      ...
                      (!toolsData || toolsData.length === 0) && (
                        <div className="rounded-lg border border-border bg-card p-4 text-center">
                          ...
                          <p className="mb-1 text-xs font-medium">{t("mcpServers.toolsViewer.noTools")}</p>
                          <p className="text-xs text-muted-foreground">{t("mcpServers.toolsViewer.noToolsServer")}</p>
                        </div>
                      )}
                      ...
                      <>
                        {filteredTools.length === 0 ? (
                          <div className="rounded-lg border border-border bg-card p-4 text-center">
                            ...
                            <p className="mb-1 text-xs font-medium">{t("mcpServers.toolsViewer.noToolsFound")}</p>
                            <p className="text-xs text-muted-foreground">{t("mcpServers.toolsViewer.noMatch", { term: toolSearchTerm })}</p>
                          </div>
                        ) : (
                                ...
                                {selectedTool?.name === tool.name && (
                                  <div className="mt-2 border-t border-border pt-2">
                                    <div className="flex items-center text-xs font-medium text-primary">
                                      ...
                                      {t("mcpServers.toolsViewer.selected")}
                                    </div>
                                  </div>
                                )}
                              </div>
          ...
          <div className="col-span-3 flex flex-col">
            <div className="flex items-center justify-between border-b border-border p-4">
              <h2 className="mb-0 text-xl font-semibold">{t("mcpServers.toolsViewer.playground")}</h2>
            </div>
