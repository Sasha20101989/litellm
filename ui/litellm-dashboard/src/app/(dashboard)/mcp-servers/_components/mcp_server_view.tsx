    ...
    <div className="max-w-full p-4">
      <div className="mb-6">
        <Button variant="ghost" className="mb-4" onClick={onBack}>
          <ArrowLeft />
          {t("mcpServers.view.back")}
        </Button>
        <div className="flex items-center gap-2">
          <h1 className="text-2xl font-semibold">{mcpServer.server_name || mcpServer.alias || t("mcpServers.view.unnamed")}</h1>
          <Button
            variant="ghost"
            size="icon-sm"
            aria-label={t("mcpServers.view.copyName")}
        ...
        <div className="mt-1 flex items-center gap-1.5">
          ...
          <Button
            variant="ghost"
            size="icon-sm"
            aria-label={t("mcpServers.view.copyId")}
      ...
      <Tabs value={String(selectedTabIndex)} onValueChange={(v: unknown) => setSelectedTabIndex(Number(v))}>
        ...
        <TabsContent value="0" keepMounted>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Card className="p-4">
              <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">{t("mcpServers.view.transport")}</p>
              <div className="mt-3">
            ...
            <Card className="p-4">
              <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">{t("mcpServers.view.authentication")}</p>
              <div className="mt-3">{getAuthBadge(handleAuth(mcpServer.auth_type ?? undefined))}</div>
            </Card>

            <Card className="p-4">
              <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">{t("mcpServers.view.hostUrl")}</p>
              <div className="mt-3 flex items-center gap-2">
                ...
                {hasToken && isProxyAdmin && (
                  <Button
                    variant="ghost"
                    size="icon-sm"
                    aria-label={showFullUrl ? t("mcpServers.view.hideUrl") : t("mcpServers.view.showUrl")}
          ...
          <Card className="mt-4 p-4">
            <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">{t("mcpServers.view.costConfiguration")}</p>
            <div className="mt-3">
            ...
            {editing ? (
              ...
              <div className="divide-y divide-border">
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.serverName")}</p>
                  <div className="col-span-2 text-sm">
                    {mcpServer.server_name || <span className="text-muted-foreground">—</span>}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.alias")}</p>
                  <div className="col-span-2 font-mono text-sm">
                    {mcpServer.alias || <span className="text-muted-foreground">—</span>}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.description")}</p>
                  <div className="col-span-2 text-sm">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.transport")}</p>
                  <div className="col-span-2">
                    {getTransportBadge(handleTransport(mcpServer.transport, mcpServer.spec_path))}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.authentication")}</p>
                  <div className="col-span-2">{getAuthBadge(handleAuth(mcpServer.auth_type))}</div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.extraHeaders")}</p>
                  <div className="col-span-2 text-sm">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.allowAllKeys")}</p>
                  <div className="col-span-2">
                    {mcpServer.allow_all_keys ? (
                      <Badge variant="outline">
                        <span className="h-1.5 w-1.5 rounded-full bg-success" />
                        {t("mcpServers.view.enabled")}
                      </Badge>
                    ) : (
                      <Badge variant="outline">{t("mcpServers.view.disabled")}</Badge>
                    )}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                ...
                {handleAuth(mcpServer.auth_type) === "oauth2" && (
                  <div className="grid grid-cols-3 gap-4 py-3">
                    <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.delegateAuth")}</p>
                    <div className="col-span-2">
                      {mcpServer.delegate_auth_to_upstream ? (
                        <Badge variant="outline">
                          <span className="h-1.5 w-1.5 rounded-full bg-success" />
                          {t("mcpServers.view.pkcePassthrough")}
                        </Badge>
                      ) : (
                        <Badge variant="outline">{t("mcpServers.view.disabled")}</Badge>
                      )}
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.accessGroups")}</p>
                  <div className="col-span-2">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.allowedTools")}</p>
                  <div className="col-span-2">
                    {mcpServer.allowed_tools && mcpServer.allowed_tools.length > 0 ? (
                      ...
                      <Badge variant="outline">{t("mcpServers.view.allTools")}</Badge>
                    )}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("mcpServers.view.cost")}</p>
                  <div className="col-span-2">
                    <MCPServerCostDisplay costConfig={mcpServer.mcp_info?.mcp_server_cost_info} />
                  </div>
                </div>
