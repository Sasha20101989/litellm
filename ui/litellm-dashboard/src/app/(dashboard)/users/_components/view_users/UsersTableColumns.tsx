...
function UserRowActions({ user, onUserClick, onDeleteUser, onResetPassword, t }: UserRowActionsProps) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        aria-label={t("users.actions.open")}
      ...
      <DropdownMenuContent align="end" className="w-48">
        ...
        <DropdownMenuItem
          onClick={() => void copyToClipboard(user.user_id, t("users.actions.idCopied"))}
...
export const getUsersTableColumns = ({
  ...
  const baseColumns: ColumnDef<UserInfo>[] = [
    {
      id: "user_id",
      accessorKey: "user_id",
      meta: { title: t("users.fields.userId") },
      header: ({ column }) => <DataTableSortHeader column={column} title={t("users.fields.userId")} variant="header-cycle" />,
      size: 220,
      enableSorting: true,
      cell: ({ row }) => (
        <IdentityCell
    ...
    },
    {
      ...
      meta: { title: t("users.fields.email") },
      header: ({ column }) => <DataTableSortHeader column={column} title={t("users.fields.email")} variant="header-cycle" />,
      size: 220,
      enableSorting: true,
      cell: ({ row }) => (
    ...
    {
      id: "status",
      meta: { title: t("users.fields.status"), skeleton: "badge" },
        ...
        if (isScimInactive(row.original)) {
          return (
            <StatusBadge
              ...
              tooltip={t("users.hints.scimInactive")}
              dataTestId={`user-status-${row.original.user_id}`}
            />
          );
        }
        return <StatusBadge tone="success" label={t("users.status.active")} dataTestId={`user-status-${row.original.user_id}`} />;
      },
    },
    {
      id: "user_role",
      accessorKey: "user_role",
      meta: { title: t("users.fields.globalRole") },
      header: ({ column }) => <DataTableSortHeader column={column} title={t("users.fields.globalRole")} variant="header-cycle" />,
      size: 160,
      enableSorting: true,
    ...
    },
    {
      ...
      meta: { title: t("users.fields.alias") },
      header: t("users.fields.alias"),
      size: 150,
      enableSorting: false,
      cell: ({ row }) => (
    ...
    },
    {
      id: "spend",
      accessorKey: "spend",
      meta: { title: t("users.fields.spend"), numeric: true },
      header: ({ column }) => <DataTableSortHeader column={column} title={t("users.fields.spend")} variant="header-cycle" />,
      size: 130,
      enableSorting: true,
    ...
    },
    {
      ...
      meta: { title: t("users.fields.budget"), numeric: true },
      header: t("users.fields.budget"),
      size: 130,
      enableSorting: false,
    ...
    },
    {
      ...
      meta: { title: t("users.fields.ssoId") },
      header: () => (
        <span className="flex items-center gap-1.5">
          {t("users.fields.ssoId")}
          <CellTooltip
            content={t("users.hints.ssoId")}
            trigger={<Info className="size-3.5 shrink-0 text-muted-foreground" aria-label={t("users.hints.aboutSsoId")} />}
          />
        </span>
      ),
      size: 160,
    ...
    },
    {
      ...
      meta: { title: t("users.fields.virtualKeys"), skeleton: "badge" },
      header: t("users.fields.virtualKeys"),
      size: 120,
      enableSorting: false,
      cell: ({ row }) => {
        ...
        if (keyCount > 0) {
          return (
            ...
            >
              {t(keyCount === 1 ? "users.keys.one" : "users.keys.many", { count: keyCount })}
            </Badge>
          );
        }
        return (
    ...
    },
    {
      ...
      meta: { title: t("users.fields.createdAt") },
      header: ({ column }) => <DataTableSortHeader column={column} title={t("users.fields.createdAt")} variant="header-cycle" />,
      size: 130,
      enableSorting: true,
    ...
    },
    {
      ...
      meta: { title: t("users.fields.updatedAt") },
      header: t("users.fields.updatedAt"),
      size: 130,
      enableSorting: false,
    ...
    {
      id: "actions",
      meta: { title: t("users.fields.actions"), className: "text-right", headerClassName: "text-right" },
      header: () => <span className="sr-only">{t("users.fields.actions")}</span>,
      size: 60,
      enableSorting: false,
      enableHiding: false,
      cell: ({ row }) => (
  ...
  return [
    createSelectionColumn<UserInfo>({
      rowAriaLabel: (row) => t("users.selection.selectUser", { label: row.original.user_email || row.original.user_id }),
    }),
    ...baseColumns,
  ];
};
