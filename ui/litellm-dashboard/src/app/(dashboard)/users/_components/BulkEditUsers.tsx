  ...
  const handleSubmit = async (formValues: any) => {
    ...
    try {
      ...
      if (!hasUserUpdates && !hasTeamAdditions) {
        toast.fromError(t("users.bulk.modifyRequired"));
        return;
      }
      ...
      if (hasTeamAdditions) {
        ...
        if (successfulTeams.length > 0) {
          ...
          successMessages.push(
            t("users.bulk.addedToTeams", { teamCount: successfulTeams.length, additionCount: totalAdditions }),
          );
        }

        if (failedTeams.length > 0) {
          toast.warning(t("users.bulk.teamAddFailed", { count: failedTeams.length }));
        }
      }
      ...
      toast.fromError(t("users.bulk.operationFailed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        <DialogHeader>
          <DialogTitle>
            {updateAllUsers ? t("users.bulk.titleAll") : t("users.bulk.titleSelected", { count: selectedUsers.length })}
          </DialogTitle>
        </DialogHeader>
        {allowAllUsers && (
          <div className="mb-4">
        ...
        {!updateAllUsers && (
          <div className="mb-4">
            <h5 className="mb-2 text-sm font-semibold text-foreground">{t("users.bulk.selectedUsers", { count: selectedUsers.length })}</h5>
            <div className="max-h-[200px] overflow-y-auto rounded-md border border-border">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[30%]">{t("users.fields.userId")}</TableHead>
                    <TableHead className="w-[25%]">{t("users.fields.email")}</TableHead>
                    <TableHead className="w-[25%]">{t("users.bulk.currentRole")}</TableHead>
                    <TableHead className="w-[20%]">{t("users.fields.budget")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {selectedUsers.map((user) => (
                    <TableRow key={user.user_id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">{user.user_email || t("users.bulk.noEmail")}</TableCell>
                      <TableCell className="text-xs text-foreground">
                        {possibleUIRoles?.[user.user_role]?.ui_label || user.user_role}
                      </TableCell>
                      <TableCell>
                        <MoneyCell value={user.max_budget} decimals={2} emptyText={t("users.keys.unlimited")} showZero />
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        <Card size="sm" className="mb-4 bg-muted/50">
          <CardHeader>
            <CardTitle>{t("users.bulk.teamManagement")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  aria-label={t("users.bulk.addToTeams")}
                />
