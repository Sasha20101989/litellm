...
const SearchConnectionTest: React.FC<SearchConnectionTestProps> = ({ litellmParams, accessToken, onTestComplete }) => {
    ...
    const runTest = async () => {
      ...
      try {
        ...
        if (result.status === "success") {
          toast.success(t("searchTools.connection.successNotification"));
        }
      } catch (error) {
        setTestResult({
          status: "error",
  ...
  const errorMessage = testResult?.message ? getCleanErrorMessage(testResult.message) : t("searchTools.connection.unknown");
      ...
      <div className="rounded-lg bg-card p-6">
        <div className="flex flex-col items-center justify-center px-5 py-8">
          ...
          <p className="text-base text-foreground">
            {t("searchTools.connection.testing", { provider: litellmParams.search_provider || t("searchTools.connection.providerFallback") })}
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-center justify-center px-5 py-8">
          ...
          <div className="ml-3">
            <p className="text-lg font-medium text-success">
              {t("searchTools.connection.success", { provider: litellmParams.search_provider })}
            </p>
            {testResult.test_query && (
              <p className="mt-2 text-sm text-muted-foreground">
                {t("searchTools.connection.testQuery")} <code className="rounded bg-muted px-1.5 py-0.5">{testResult.test_query}</code>
              </p>
            )}
            {testResult.results_count !== undefined && (
              <p className="text-sm text-muted-foreground">{t("searchTools.connection.resultsRetrieved", { count: testResult.results_count })}</p>
            )}
          </div>
        </div>
      ) : (
        <div>
          <div className="mb-5 flex items-center">
            ...
            <p className="text-lg font-medium text-destructive">
              {t("searchTools.connection.failed", { provider: litellmParams.search_provider || t("searchTools.connection.providerFallback") })}
            </p>
          </div>
          ...
          {showDetails && (
            <div className="mb-5">
              <p className="mb-2 text-[15px] font-semibold text-foreground">{t("searchTools.connection.fullDetails")}</p>
          ...
          <div className="rounded-lg border border-warning/20 border-l-4 border-l-amber-500 bg-warning/10 p-4">
            <p className="mb-2 font-semibold text-warning">{t("searchTools.connection.troubleshooting")}</p>
