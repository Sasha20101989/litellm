...
const SearchTools: React.FC<SearchToolsProps> = ({ accessToken, userRole, userID }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("searchTools.deleted"));
      setIsDeleteModalOpen(false);
      setToolToDelete(null);
      refetch();
    } catch (error) {
      console.error("Error deleting the search tool:", error);
      toast.error(t("searchTools.deleteFailed"));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const submitEdit = form.handleSubmit(
    async (values) => {
      ...
      try {
        ...
        toast.success(t("searchTools.updated"));
        ...
        toast.error(t("searchTools.updateFailed"));
      }
    },
    (errors) => {
  ...
  const renderEditForm = () => (
    <form onSubmit={(event) => event.preventDefault()}>
      <FieldGroup>
        <FormField control={form.control} name="search_tool_name" label={t("searchTools.edit.name")}>
          {({ ref, ...field }) => <Input {...field} ref={ref} placeholder={t("searchTools.edit.namePlaceholder")} />}
        </FormField>

        <FormField control={form.control} name="search_provider" label={t("searchTools.edit.provider")}>
          {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
            ...
            >
              <SelectTrigger id={id} aria-invalid={ariaInvalid} aria-describedby={ariaDescribedBy} className="w-full">
                <SelectValue placeholder={t("searchTools.edit.providerPlaceholder")} />
        ...
        <FormField control={form.control} name="api_key" label={t("searchTools.edit.apiKey")} description={t("searchTools.edit.apiKeyHint")}>
          {({ ref, value, ...field }) => (
            <PasswordInput {...field} ref={ref} value={value ?? ""} placeholder={t("searchTools.edit.apiKeyPlaceholder")} />
          )}
        </FormField>

        <FormField control={form.control} name="description" label={t("searchTools.edit.description")}>
          {({ ref, value, ...field }) => (
            <Textarea {...field} ref={ref} value={value ?? ""} rows={3} placeholder={t("searchTools.edit.descriptionPlaceholder")} />
          )}
        </FormField>
      </FieldGroup>
    </form>
  ...
  if (!accessToken || !userRole || !userID) {
    return <div className="p-6 text-center text-muted-foreground">{t("searchTools.missingAuth")}</div>;
  }
    ...
    <div className="w-full h-full p-6">
        ...
        resourceInformation={
          toolToDelete
            ? [
                ...
                { label: t("searchTools.deleteModal.description"), value: toolToDelete.search_tool_info?.description || "-" },
              ]
            : []
        }
      ...
      >
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>{t("searchTools.edit.title")}</DialogTitle>
          </DialogHeader>
          {renderEditForm()}
          <DialogFooter>
            <Button
            ...
            >
              {t("searchTools.cancel")}
            </Button>
            <Button onClick={handleEditSubmit}>{t("searchTools.ok")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <h1 className="text-lg font-semibold text-foreground">{t("searchTools.title")}</h1>
      <p className="mt-2 text-sm text-muted-foreground">{t("searchTools.subtitle")}</p>
