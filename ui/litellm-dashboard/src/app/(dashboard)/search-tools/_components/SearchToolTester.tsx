...
export const SearchToolTester: React.FC<SearchToolTesterProps> = ({ searchToolName, accessToken, className = "" }) => {
  ...
  const handleSearch = async () => {
    if (!query.trim()) {
      toast.warning(t("searchTools.tester.queryRequired"));
      return;
    }
    ...
    try {
      ...
      toast.fromError(t("searchTools.tester.queryFailed"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const clearHistory = () => {
    setSearchHistory([]);
    setExpandedResults({});
    toast.success(t("searchTools.tester.historyCleared"));
  };
                        ...
                        <div className="mt-1 flex items-center gap-3">
                          <div className="text-sm font-semibold text-primary">
                            ...
                            {latestResults.response?.results?.length === 1
                              ? t("searchTools.tester.resultOne", { count: latestResults.response?.results?.length || 0 })
                              : t("searchTools.tester.resultMany", { count: latestResults.response?.results?.length || 0 })}
                          </div>
                          ...
                          >
                            <div className="p-5">
                              ...
                              {result.snippet.length > 200 && (
                                ...
                                >
                                  {isResultExpanded ? t("searchTools.tester.showLess") : t("searchTools.tester.showMore")}
                                </Button>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="mt-1.5 flex items-center gap-2 text-xs text-muted-foreground">
                          <span className="font-medium text-primary">
                            ...
                            {entry.response?.results?.length === 1
                              ? t("searchTools.tester.resultOne", { count: entry.response?.results?.length || 0 })
                              : t("searchTools.tester.resultMany", { count: entry.response?.results?.length || 0 })}
                          </span>
