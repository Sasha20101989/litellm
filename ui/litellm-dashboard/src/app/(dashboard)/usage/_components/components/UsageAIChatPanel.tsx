  ...
  return (
    <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-gray-100 border border-gray-200 text-xs">
      ...
      <div className="min-w-0">
        ...
        {filter && <div className="text-gray-500 mt-0.5">{t("aiChat.filter", { value: filter })}</div>}
    ...
    try {
      await usageAiChatStream(
        ...
        (errorMsg: string) => {
          setStatusMessage(null);
          setActiveToolCalls([]);
          setMessages((prev) => [...prev, { role: "assistant", content: t("aiChat.error", { message: errorMsg }) }]);
          setStreamingContent("");
        },
        (status: string) => {
      ...
      const errorMsg = error?.message || t("aiChat.genericError");
      setMessages((prev) => [...prev, { role: "assistant", content: t("aiChat.error", { message: errorMsg }) }]);
      setStreamingContent("");
    } finally {
      setIsLoading(false);
    ...
    >
      ...
      <div className="px-5 pt-5 pb-3 border-b border-gray-100 shrink-0">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-gray-900">{t("aiChat.title")}</h3>
          </div>
          <button
            onClick={handleClose}
        ...
        <p className="text-xs text-gray-500">{t("aiChat.subtitle")}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
        {messages.length === 0 && !streamingContent && !isLoading && (
          <div className="flex flex-col items-center justify-center h-full text-gray-400">
            ...
            <p className="text-sm font-medium">{t("aiChat.emptyTitle")}</p>
            <p className="text-xs mt-1">{t("aiChat.example")}</p>
          </div>
        )}
        ...
        {isLoading && !streamingContent && (
          <div className="flex items-center gap-2 px-3 py-2 text-xs text-gray-500">
            <Spin size="small" />
            <span className="italic">{statusMessage || t("aiChat.thinking")}</span>
          </div>
        )}
      ...
      <div className="px-4 py-3 border-t border-gray-200 bg-white shrink-0">
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <span className="text-xs text-gray-400">{t("aiChat.enterToSend")}</span>
        </div>
      </div>
    </div>
  );
