...
function transformDailyDataToChart(dailyData: DailyData[], locale = "en-US"): Array<Record<string, string | number>> {
...
export function EndpointUsageLineChart({ dailyData }: EndpointUsageLineChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    return transformDailyDataToChart(dailyData.results, i18n.language === "ru" ? "ru-RU" : "en-US");
  ...
  return (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle className="text-base font-semibold">{t("endpoint.trends")}</CardTitle>
      </CardHeader>
      <CardContent>
        <LineChart
          className="h-80"
