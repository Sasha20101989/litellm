...
const EntityUsage: React.FC<EntityUsageProps> = ({
  ...
  const getFilterLabel = (entityType: string) => {
    return t("entity.filterBy", { entity: t(`entity.labels.${entityType}`) });
  };

  const getFilterPlaceholder = (entityType: string) => {
    return t("entity.selectToFilter", { entity: t(`entity.labels.${entityType}`) });
  };

  const capitalizedEntityLabel = t(`entity.labels.${entityType}`);
  ...
  const breakdownTiles = showFlatCost && showCostBreakdown ? buildCostBreakdownTiles(spendData.metadata, t) : [];
  const summaryTiles = [...buildSummaryTiles(spendData.metadata, showFlatCost, t), ...breakdownTiles];

  const modelViewTitle = modelViewType === "groups" ? t("common.topPublicModels") : t("common.topLiteLLMModels");
  const requestCostLabel = t("entity.requestCost");
  const flatCostLabel = t("entity.flatCost");

  const costPanel = (
    <Grid numItems={2} className="gap-2 w-full">
      <Col numColSpan={2}>
        <Card>
          <Title>{t("entity.spendOverview", { entity: capitalizedEntityLabel })}</Title>
      ...
      <Col numColSpan={2}>
        <ShadcnCard>
          <CardHeader>
            <CardTitle className="text-base font-semibold">{t("common.dailySpend")}</CardTitle>
          </CardHeader>
          <CardContent>
            <BarChart
              data={[...spendData.results]
                ...
                .map((row) => ({
                  ...row,
                  [requestCostLabel]: row.metrics.spend ?? 0,
                  [flatCostLabel]: row.metrics.flat_cost ?? 0,
                }))}
              index="date"
              categories={showFlatCost ? [requestCostLabel, flatCostLabel] : ["metrics.spend"]}
                ...
                return (
                  <div className="bg-white p-4 shadow-lg rounded-lg border">
                    ...
                    <div className="mt-2 border-t pt-2">
                      <p className="font-semibold">{t("entity.spendBy", { entity: capitalizedEntityLabel })}</p>
                      ...
                      {entityCount > 5 && (
                        <p className="text-sm text-gray-500 italic">{t("entity.more", { count: entityCount - 5 })}</p>
                      )}
                    </div>
                  </div>
                );
        ...
        <Card>
          <div className="flex flex-col space-y-4">
            <div className="flex flex-col space-y-2">
              <Title>{t("entity.spendPer", { entity: capitalizedEntityLabel })}</Title>
              <Subtitle className="text-xs">{t("entity.showingTopFive")}</Subtitle>
              <div className="flex items-center text-sm text-gray-500">
                <span>{t("entity.trackingPrefix", { entity: capitalizedEntityLabel })} </span>
                <a
      ...
      <Col numColSpan={1}>
        <Card>
          <div className="flex justify-between items-center">
            <Title>{entityType === "agent" ? t("common.topAgents") : modelViewTitle}</Title>
      ...
      {showAgentBreakdown && (
        <Col numColSpan={2}>
          <Card>
            <Title>{t("entity.topAgentsDrivingSpend")}</Title>
            <TopModelView
      ...
      <Col numColSpan={2}>
        <Card>
          <div className="flex flex-col space-y-4">
            <Title>{t("entity.providerUsage")}</Title>
            <Grid numItems={2}>
              <Col numColSpan={1}>
                <DonutChart
                  className="mt-4 h-40"
  ...
  const tabs: readonly { key: string; label: string; content: ReactNode }[] = [
    { key: "cost", label: t("common.cost"), content: costPanel },
    {
      key: "models",
      label: entityType === "agent" ? t("common.requestTokenConsumption") : t("common.modelActivity"),
      content: (
        <>
    ...
    ...(showAgentBreakdown
      ? [{ key: "agents", label: t("common.agentActivity"), content: <ActivityMetrics modelMetrics={agentMetrics} /> }]
      : []),
    {
      key: "keys",
      label: t("common.keyActivity"),
      content: <ActivityMetrics modelMetrics={keyMetrics} hidePromptCachingMetrics={entityType === "agent"} />,
    },
    { key: "endpoints", label: t("common.endpointActivity"), content: <EndpointUsage userSpendData={spendData} /> },
  ];
          ...
          message={
            <div className="flex items-center justify-between">
              <span>
                ...
                {t("entity.fetchingSpend", { current: progress.currentPage, total: progress.totalPages })}{" "}
                <a href={window.location.href} target="_blank" rel="noopener noreferrer">
                  {t("common.openNewTab")} <ExportOutlined />
                </a>
                .
              </span>
