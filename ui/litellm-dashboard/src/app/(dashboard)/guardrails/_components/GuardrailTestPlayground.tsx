...
const GuardrailTestPlayground: React.FC<GuardrailTestPlaygroundProps> = ({
  ...
  const handleTestGuardrails = async (text: string, metadata?: Record<string, unknown> | null) => {
    ...
    if (results.length > 0) {
      toast.success(t("guardrailsPage.playground.notifications.applied", { count: results.length }));
    }
    if (errors.length > 0) {
      toast.fromError(t("guardrailsPage.playground.notifications.failed", { count: errors.length }));
    }
  };
          ...
          <div className="flex h-full">
            ...
            <div className="flex w-1/4 flex-col overflow-hidden border-r border-border">
              <div className="border-b border-border p-4">
                <div className="mb-3">
                  <h3 className="mb-3 text-lg font-semibold">{t("guardrailsPage.playground.guardrails")}</h3>
                  <InputGroup>
                    <InputGroupAddon>
                      <Search className="size-4 text-muted-foreground" />
                    </InputGroupAddon>
                    <InputGroupInput
                      placeholder={t("guardrailsPage.playground.searchPlaceholder")}
                      value={searchQuery}
              ...
              <div className="flex-1 overflow-auto">
                {isLoading ? (
                  ...
                  <div className="p-4 text-center text-muted-foreground">
                    {searchQuery
                      ? t("guardrailsPage.playground.noSearchResults")
                      : t("guardrailsPage.playground.noGuardrails")}
                  </div>
                ) : (
                    ...
                    {filteredGuardrails.map((guardrail) => (
                      ...
                      >
                        ...
                        <div className="mt-1 space-y-1 text-xs">
                          <div>
                            <span className="font-medium">{t("guardrailsPage.playground.type")}: </span>
                            <span className="text-muted-foreground">{guardrail.litellm_params.guardrail}</span>
                          </div>
                          <div>
                            <span className="font-medium">{t("guardrailsPage.playground.mode")}: </span>
              ...
              <div className="border-t border-border bg-muted/40 p-3">
                <span className="text-xs text-muted-foreground">
                  {t("guardrailsPage.playground.selected", {
            ...
            <div className="flex w-3/4 flex-col">
              <div className="flex items-center justify-between border-b border-border p-4">
                <h2 className="mb-0 text-xl font-semibold">{t("guardrailsPage.playground.title")}</h2>
              </div>
              ...
              <div className="flex-1 overflow-auto p-4">
                {selectedGuardrails.size === 0 ? (
                  <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
                    ...
                    <p className="mb-2 text-lg font-medium">{t("guardrailsPage.playground.selectTitle")}</p>
                    <p className="max-w-md text-center">
                      {t("guardrailsPage.playground.selectDescription")}
                    </p>
                  </div>
                ) : (
