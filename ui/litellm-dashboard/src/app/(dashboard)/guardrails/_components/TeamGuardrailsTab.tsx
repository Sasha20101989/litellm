...
const createSubmitGuardrailSchema = (t: TFunction) =>
  z.object({
    team_id: z
      .string()
      .nullable()
      .pipe(
        z
          .string({ error: t("guardrailsPage.submissions.form.selectTeam") })
          .min(1, t("guardrailsPage.submissions.form.selectTeam")),
      ),
    guardrail_name: z.string().min(1, t("guardrailsPage.submissions.form.enterName")),
    mode: z.string().min(1, t("guardrailsPage.submissions.form.selectMode")),
    api_base: z
      .string()
      .min(1, t("guardrailsPage.submissions.form.enterApiBase"))
      .refine(isValidUrl, t("guardrailsPage.submissions.form.validUrl")),
    extra_litellm_params: z.string().superRefine((value, ctx) => {
      if (!value) return;
      try {
        const parsed: unknown = JSON.parse(value);
        if (typeof parsed !== "object" || Array.isArray(parsed)) {
          ctx.addIssue({ code: "custom", message: t("guardrailsPage.submissions.form.jsonObject") });
        }
      } catch {
        ctx.addIssue({ code: "custom", message: t("guardrailsPage.submissions.form.invalidJson") });
      }
    }),
...
type SubmitGuardrailValues = z.output<ReturnType<typeof createSubmitGuardrailSchema>>;
...
const STATUS_CONFIG: Record<GuardrailStatus, { bg: string; text: string; dot: string }> = {
  active: {
    bg: "bg-success/10",
    text: "text-success",
    dot: "bg-success",
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <span>
              {t("guardrailsPage.submissions.model")}: <span className="font-medium text-foreground">{g.model}</span>
            </span>
            <span>
              {t("guardrailsPage.submissions.submitted")}:{" "}
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground whitespace-nowrap">
              {t("guardrailsPage.submissions.forwardApiKey")}
            </span>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            >
              {isSelected ? t("guardrailsPage.submissions.close") : t("guardrailsPage.submissions.review")}
            </button>
            {isAdmin && g.status === "pending" && (
              <>
                ...
                >
                  {t("guardrailsPage.submissions.approve")}
                </button>
                <button
                  type="button"
                  onClick={onReject}
                ...
                >
                  {t("guardrailsPage.submissions.reject")}
                </button>
              </>
            )}
          </div>
      ...
      <div className="mt-3 pt-3 border-t border-border">
        ...
        >
          ...
          {t("guardrailsPage.submissions.staticHeaders")}
        ...
        {isHeadersExpanded && (
          <div className="mt-2">
            {g.customHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic">{t("guardrailsPage.submissions.noStaticHeaders")}</p>
            ) : (
    ...
    <div className="w-96 shrink-0 bg-card overflow-auto">
      <div className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {t("guardrailsPage.submissions.submittedBy", { user: g.submittedBy, date: g.submittedAt })}
            </p>
          </div>
          <button
            type="button"
            ...
            aria-label={t("guardrailsPage.submissions.closeDetails")}
          >
        ...
        <div className="space-y-4">
          <ConfigRow label={t("guardrailsPage.submissions.endpoint")}>
          ...
          <ConfigRow label={t("guardrailsPage.submissions.method")}>
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-info">
                  {t("guardrailsPage.submissions.forwardLitellmKey")}
                </span>
              </div>
              <Toggle enabled={g.forwardKey} onToggle={onToggleForwardKey} disabled={!isAdmin} />
            </div>
            <p className="text-xs text-info leading-relaxed">
              {t("guardrailsPage.submissions.forwardKeyDescriptionPrefix")}{" "}
              <code className="font-mono bg-info/15 px-1 rounded-sm">Authorization</code>{" "}
              {t("guardrailsPage.submissions.forwardKeyDescriptionSuffix")}
            </p>
          </div>
          <div>
            <div className="flex items-center gap-1.5 mb-2">
            ...
            <p className="text-xs text-muted-foreground mb-2">{t("guardrailsPage.submissions.staticHeadersHelp")}</p>
            {g.customHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic mb-2">
                {t("guardrailsPage.submissions.noStaticHeaders")}
              </p>
            ) : (
                  ...
                  >
                    ...
                    {isAdmin && (
                      <button
                        ...
                        aria-label={t("guardrailsPage.submissions.removeHeader", { name: h.key })}
                      >
            ...
            {isAdmin && (
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
                <input
                  ...
                  placeholder={t("guardrailsPage.submissions.headerNamePlaceholder")}
                ...
                <input
                  ...
                  placeholder={t("guardrailsPage.submissions.valuePlaceholder")}
                  className="flex-1 min-w-0 text-xs font-mono border border-border rounded-sm px-2 py-1.5 text-foreground placeholder:text-muted-foreground focus:outline-hidden focus:ring-1 focus:ring-ring"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
                ...
                >
                  {t("guardrailsPage.submissions.add")}
                </button>
              </div>
            )}
          </div>
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-xs font-semibold text-foreground">
                {t("guardrailsPage.submissions.forwardClientHeaders")}
              </span>
            ...
            <p className="text-xs text-muted-foreground mb-2">
              {t("guardrailsPage.submissions.forwardClientHeadersHelp")}
            </p>
            {g.extraHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic mb-2">
                {t("guardrailsPage.submissions.noForwardClientHeaders")}
              </p>
            ) : (
              <ul className="list-none space-y-1 mb-2">
                  ...
                  >
                    ...
                    {isAdmin && (
                      <button
                        ...
                        className="text-muted-foreground hover:text-destructive shrink-0"
                        aria-label={t("guardrailsPage.submissions.removeHeader", { name })}
                      >
                        <XIcon className="h-3.5 w-3.5" />
                      </button>
                    )}
            ...
            {isAdmin && (
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={t("guardrailsPage.submissions.clientHeaderPlaceholder")}
                  className="flex-1 min-w-0 text-xs font-mono border border-border rounded-sm px-2 py-1.5 text-foreground placeholder:text-muted-foreground focus:outline-hidden focus:ring-1 focus:ring-ring"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
          ...
          <div className="border border-border rounded-lg overflow-hidden">
            ...
            >
              <span>{t("guardrailsPage.submissions.equivalentConfig")}</span>
              {configExpanded ? (
          ...
          <div className="flex items-start gap-2 bg-muted border border-border rounded-lg p-3">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              {t("guardrailsPage.submissions.separateInstancePrefix")}{" "}
              <a
              ...
              >
                {t("guardrailsPage.submissions.documentation")}
              </a>{" "}
              {t("guardrailsPage.submissions.separateInstanceSuffix")}
            </p>
          </div>
        </div>
        <div className="mt-5 pt-4 border-t border-border space-y-2">
          ...
          >
            ...
            {t("guardrailsPage.submissions.testEndpoint")}
          </button>
          {isAdmin && g.status === "pending" && (
            <div className="flex gap-2">
              <button
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-overlay">
      <div className="bg-card rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
        ...
        <h3 className="text-base font-semibold text-foreground mb-1">
          {isApprove
            ? t("guardrailsPage.submissions.confirm.approveTitle")
            : t("guardrailsPage.submissions.confirm.rejectTitle")}
        </h3>
        <p className="text-sm text-muted-foreground mb-5">
          {isApprove
            ? t("guardrailsPage.submissions.confirm.approveQuestion", { name: guardrailName })
            : t("guardrailsPage.submissions.confirm.rejectQuestion", { name: guardrailName })}{" "}
          {isApprove
            ? t("guardrailsPage.submissions.confirm.approveHelp")
            : t("guardrailsPage.submissions.confirm.rejectHelp")}
        </p>
        <div className="flex gap-3">
          ...
          >
            {t("guardrailsPage.submissions.cancel")}
          </button>
          <button
            type="button"
            onClick={onConfirm}
          ...
          >
            {isApprove ? t("guardrailsPage.submissions.approve") : t("guardrailsPage.submissions.reject")}
          </button>
        </div>
      </div>
    </div>
...
export function TeamGuardrailsTab({ accessToken }: TeamGuardrailsTabProps) {
  ...
  const submitGuardrailSchema = useMemo(() => createSubmitGuardrailSchema(t), [t]);
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("guardrailsPage.submissions.loadFailed"));
      setGuardrails([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmitGuardrail = submitForm.handleSubmit(async (values) => {
    ...
    try {
      ...
      toast.success(t("guardrailsPage.submissions.notifications.submitted"));
  ...
  async function toggleForwardKey(id: string) {
    ...
    try {
      ...
      toast.success(
        newValue
          ? t("guardrailsPage.submissions.notifications.forwardKeyEnabled")
          : t("guardrailsPage.submissions.notifications.forwardKeyDisabled"),
      );
    } catch {
      toast.fromError(t("guardrailsPage.submissions.notifications.forwardKeyFailed"));
    }
  }
  ...
  async function updateCustomHeaders(id: string, customHeaders: { key: string; value: string }[]) {
    ...
    try {
      ...
      toast.success(t("guardrailsPage.submissions.notifications.staticHeadersUpdated"));
    } catch {
      toast.fromError(t("guardrailsPage.submissions.notifications.staticHeadersFailed"));
    }
  }
  ...
  async function updateExtraHeaders(id: string, extraHeaders: string[]) {
    ...
    try {
      ...
      toast.success(t("guardrailsPage.submissions.notifications.clientHeadersUpdated"));
    } catch {
      toast.fromError(t("guardrailsPage.submissions.notifications.clientHeadersFailed"));
    }
  }
  ...
  async function handleApprove(id: string) {
    ...
    try {
      ...
      toast.success(t("guardrailsPage.submissions.notifications.approved"));
    } catch {
      toast.fromError(t("guardrailsPage.submissions.notifications.approveFailed"));
    }
  }
  ...
  async function handleReject(id: string) {
    ...
    try {
      ...
      if (selectedId === id) setSelectedId(null);
      await fetchSubmissions();
      toast.success(t("guardrailsPage.submissions.notifications.rejected"));
    } catch {
      toast.fromError(t("guardrailsPage.submissions.notifications.rejectFailed"));
    }
  }
    ...
    <div className="flex h-full">
      <div className={`flex-1 min-w-0 p-6 overflow-auto ${selected ? "border-r border-border" : ""}`}>
        <div className="grid grid-cols-4 gap-4 mb-6">
          <StatCard label={t("guardrailsPage.submissions.total")} value={totalCount} color="text-foreground" />
          <StatCard label={t("guardrailsPage.submissions.status.pending")} value={pendingCount} color="text-warning" />
          <StatCard label={t("guardrailsPage.submissions.status.active")} value={activeCount} color="text-success" />
          <StatCard
            label={t("guardrailsPage.submissions.status.rejected")}
            value={rejectedCount}
        ...
        <div className="flex items-center gap-3 mb-5">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              type="text"
              placeholder={t("guardrailsPage.submissions.searchPlaceholder")}
              value={search}
          ...
          <select
            aria-label={t("guardrailsPage.submissions.allStatuses")}
            value={statusFilter}
          ...
          >
            <option value="all">{t("guardrailsPage.submissions.allStatuses")}</option>
            <option value="pending">{t("guardrailsPage.submissions.status.pending")}</option>
            <option value="active">{t("guardrailsPage.submissions.status.active")}</option>
            <option value="rejected">{t("guardrailsPage.submissions.status.rejected")}</option>
          </select>
          <button
            type="button"
          ...
          >
            ...
            {t("guardrailsPage.submissions.addGuardrail")}
          </button>
        </div>
        <div className="space-y-3">
          {isLoading && (
            <div className="text-center py-12 text-muted-foreground text-sm">
              {t("guardrailsPage.submissions.loading")}
            </div>
          )}
          {error && <div className="text-center py-12 text-destructive text-sm">{error}</div>}
          {!isLoading && !error && filtered.length === 0 && (
            <div className="text-center py-12 text-muted-foreground text-sm">
              {t("guardrailsPage.submissions.empty")}
            </div>
          )}
          {!isLoading &&
            !error &&
      ...
      >
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("guardrailsPage.submissions.form.title")}</DialogTitle>
          </DialogHeader>
          <div className="rounded-md bg-info/10 border border-info/20 px-4 py-3 text-sm text-info mb-4">
            {t("guardrailsPage.submissions.form.reviewNotice")}
          </div>
          <TooltipProvider>
            <form onSubmit={handleSubmitGuardrail}>
              <FieldGroup>
                <FormField control={submitForm.control} name="team_id" label={t("guardrailsPage.submissions.team")}>
                  {({ id, value, onChange }) => <TeamDropdown id={id} value={value} onChange={onChange} />}
                </FormField>
                <FormField
                  control={submitForm.control}
                  name="guardrail_name"
                  label={t("guardrailsPage.submissions.form.name")}
                >
                  {({ ref, ...field }) => (
                    <Input {...field} ref={ref} placeholder={t("guardrailsPage.submissions.form.namePlaceholder")} />
                  )}
                </FormField>
                <FormField control={submitForm.control} name="mode" label={t("guardrailsPage.submissions.form.mode")}>
                  {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
                    <Select
                      items={[
                        { value: "pre_call", label: t("guardrailsPage.submissions.form.preCall") },
                        { value: "post_call", label: t("guardrailsPage.submissions.form.postCall") },
                        { value: "during_call", label: t("guardrailsPage.submissions.form.duringCall") },
                      ]}
                      value={value}
                ...
                <FormField
                  control={submitForm.control}
                  name="api_base"
                  label={t("guardrailsPage.submissions.form.apiBase")}
                >
                  {({ ref, ...field }) => (
                    <Input
                      {...field}
                      ref={ref}
                      placeholder={t("guardrailsPage.submissions.form.apiBasePlaceholder")}
                      className="font-mono"
                    />
                  )}
                </FormField>
                <FormField
                  ...
                  label={labelWithHint(
                    t("guardrailsPage.submissions.form.additionalParams"),
                    t("guardrailsPage.submissions.form.additionalParamsTooltip"),
                  )}
                >
                  {({ ref, ...field }) => (
                    <Textarea
                      ...
                      placeholder={t("guardrailsPage.submissions.form.additionalParamsPlaceholder")}
                    />
                  )}
                </FormField>
                <FormField
                  control={submitForm.control}
                  name="guardrail_info"
                  label={t("guardrailsPage.submissions.form.info")}
                >
                  {({ ref, ...field }) => (
                    <Textarea
                      {...field}
                      ...
                      placeholder={JSON.stringify({
                        description: t("guardrailsPage.submissions.form.infoPlaceholder"),
                      })}
                    />
                  )}
                </FormField>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmitGuardrail}>{t("guardrailsPage.submissions.form.submit")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
