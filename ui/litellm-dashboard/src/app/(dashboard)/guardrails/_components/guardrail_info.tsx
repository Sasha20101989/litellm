...
/* eslint-disable max-lines -- existing guardrail details form is intentionally kept in one component */
...
const GuardrailInfoView: React.FC<GuardrailInfoProps> = ({ guardrailId, onClose, accessToken, isAdmin }) => {
  ...
  const fetchGuardrailInfo = async () => {
    try {
      ...
      toast.fromError(t("guardrailsPage.details.notifications.loadFailed"));
  ...
  const handleGuardrailUpdate = async (values: GuardrailFormValues) => {
    try {
      ...
      if (Object.keys(updateData).length === 0) {
        toast.info(t("guardrailsPage.details.notifications.noChanges"));
        setIsEditing(false);
        return;
      }

      await updateGuardrailCall(accessToken, guardrailId, updateData);
      toast.success(t("guardrailsPage.details.notifications.updated"));
      setHasUnsavedContentFilterChanges(false);
      fetchGuardrailInfo();
      setIsEditing(false);
    } catch (error) {
      console.error("Error updating guardrail:", error);
      toast.fromError(t("guardrailsPage.details.notifications.updateFailed"));
    }
  };
  ...
  if (loading) {
    return <div className="p-4">{t("guardrailsPage.details.loading")}</div>;
  }

  const backButton = (
    <Button variant="ghost" onClick={onClose} className="mb-4">
      <ArrowLeft className="w-4 h-4" />
      {t("guardrailsPage.details.back")}
    </Button>
  );

  if (!guardrailData) {
    return <div className="p-4">{backButton}{t("guardrailsPage.details.notFound")}</div>;
  }
  ...
  return (
    <div className="p-4">
      <div>
        {backButton}
        <h1 className="text-2xl font-semibold">{guardrailData.guardrail_name || t("guardrailsPage.values.unnamed")}</h1>
      ...
      <Tabs defaultValue="overview">
        <TabsList variant="line" className="mb-4 h-auto w-full justify-start rounded-none border-b p-0">
          <TabsTrigger value="overview" className="flex-none rounded-none px-4 py-2">
            {t("guardrailsPage.details.overview")}
          </TabsTrigger>
          {isAdmin && (
            <TabsTrigger value="settings" className="flex-none rounded-none px-4 py-2">
              {t("guardrailsPage.details.settings")}
            </TabsTrigger>
          )}
        </TabsList>
        ...
        <div>
          ...
          <TabsContent value="overview" keepMounted>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              <Card className="block p-6">
                <p>{t("guardrailsPage.columns.provider")}</p>
              ...
              <Card className="block p-6">
                <p>{t("guardrailsPage.columns.mode")}</p>
                <div className="mt-2">
                  ...
                  <Badge variant={guardrailData.litellm_params?.default_on ? "secondary" : "outline"}>
                    {guardrailData.litellm_params?.default_on
                      ? t("guardrailsPage.values.defaultOn")
                      : t("guardrailsPage.values.defaultOff")}
                  </Badge>
                </div>
              </Card>

              <Card className="block p-6">
                <p>{t("guardrailsPage.columns.createdAt")}</p>
                <div className="mt-2">
                  <h3 className="text-lg font-medium">{formatDate(guardrailData.created_at)}</h3>
                  <p>{t("guardrailsPage.details.lastUpdated")}: {formatDate(guardrailData.updated_at)}</p>
                </div>
              </Card>
            </div>
              ...
              Object.keys(guardrailData.litellm_params.pii_entities_config).length > 0 && (
                <Card className="block mt-6 p-6">
                  <div className="flex justify-between items-center">
                    <p className="font-medium">{t("guardrailsPage.details.piiProtection")}</p>
                    <Badge variant="secondary">
                      {t("guardrailsPage.details.piiEntitiesConfigured", {
            ...
            {guardrailData.litellm_params?.pii_entities_config &&
              Object.keys(guardrailData.litellm_params.pii_entities_config).length > 0 && (
                <Card className="block mt-6 p-6">
                  <p className="mb-4 text-lg font-semibold">{t("guardrailsPage.details.piiConfiguration")}</p>
                  <div className="border rounded-lg overflow-hidden shadow-xs">
                    <div className="bg-muted px-5 py-3 border-b flex">
                      <p className="flex-1 font-semibold text-foreground">{t("guardrailsPage.details.entityType")}</p>
                      <p className="flex-1 font-semibold text-foreground">{t("guardrailsPage.details.configuration")}</p>
                    </div>
              ...
              <Card className="block mt-6 p-6">
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="font-medium text-lg">{t("guardrailsPage.details.customCode")}</p>
                  </div>
                  {isAdmin && !isConfigGuardrail && (
                    <Button variant="outline" size="sm" onClick={() => setCustomCodeModalVisible(true)}>
                      <Code />
                      {t("guardrailsPage.details.editCode")}
                    </Button>
                  )}
                </div>
          ...
          {isAdmin && (
            <TabsContent value="settings" keepMounted>
              <Card className="block p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-medium">{t("guardrailsPage.details.guardrailSettings")}</h3>
                  {isConfigGuardrail && (
                    <SimpleTooltip content={t("guardrailsPage.details.configReadOnly")}>
                      <Info role="img" aria-label={t("guardrailsPage.details.configReadOnly")} className="size-4 text-muted-foreground" />
                    </SimpleTooltip>
                  )}
                  {!isEditing &&
                    !isConfigGuardrail &&
                    (guardrailData.litellm_params?.guardrail === "custom_code" ? (
                      ...
                      <Button variant="outline" onClick={() => setIsEditing(true)}>
                        {t("guardrailsPage.details.editSettings")}
                      </Button>
                    ))}
                </div>
                ...
                {isEditing ? (
                    ...
                    <form onSubmit={form.handleSubmit(submitLatest)}>
                      <FieldGroup>
                        <GuardrailField
                          ...
                          name="guardrail_name"
                          label={t("guardrailsPage.details.guardrailName")}
                          rules={requiredRule(t("guardrailsPage.details.guardrailNameRequired"))}
                        >
                          {({ ref, value, ...field }) => (
                            <Input {...field} ref={ref} value={asText(value)} placeholder={t("guardrailsPage.details.guardrailNamePlaceholder")} />
                          )}
                        </GuardrailField>

                        <GuardrailField control={form.control} name="default_on" label={t("guardrailsPage.columns.defaultOn")}>
                          {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": describedBy }) => (
                            <Select
                              items={[
                                { label: t("guardrailsPage.values.yes"), value: true },
                                { label: t("guardrailsPage.values.no"), value: false },
                              ]}
                            ...
                            >
                              <SelectTrigger
                                ...
                                className="w-full"
                              >
                                <SelectValue placeholder={t("guardrailsPage.details.selectOption")} />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value={true}>{t("guardrailsPage.values.yes")}</SelectItem>
                                <SelectItem value={false}>{t("guardrailsPage.values.no")}</SelectItem>
                              </SelectContent>
                            </Select>
                          )}
                        </GuardrailField>
                        ...
                        <GuardrailField
                          ...
                          label={labelWithHint(
                            t("guardrailsPage.details.skipSystemMessages"),
                            t("guardrailsPage.details.skipSystemMessagesHelp"),
                          )}
                        >
                        ...
                        <GuardrailField
                          ...
                          label={labelWithHint(
                            t("guardrailsPage.details.skipToolMessages"),
                            t("guardrailsPage.details.skipToolMessagesHelp"),
                          )}
                        >
                          {(fieldControl) => <SkipMessageSelect control={fieldControl} />}
                        </GuardrailField>
                        {guardrailData.litellm_params?.guardrail === "presidio" && (
                          <>
                            <SectionHeading>{t("guardrailsPage.details.piiProtection")}</SectionHeading>
                            <div className="mb-6">
                              {guardrailSettings && (
                                <PiiConfiguration
                        ...
                        {(guardrailData.litellm_params?.guardrail === "tool_permission" ||
                          guardrailProviderSpecificParams) && <SectionHeading>{t("guardrailsPage.details.providerSettings")}</SectionHeading>}
                        ...
                        <SectionHeading>{t("guardrailsPage.details.advancedSettings")}</SectionHeading>
                        <GuardrailField control={form.control} name="guardrail_info" label={t("guardrailsPage.details.guardrailInformation")}>
                          {({ ref, value, ...field }) => (
                        ...
                        <div className="mt-6 flex justify-end gap-2">
                          ...
                          >
                            {t("guardrailsPage.details.cancel")}
                          </Button>
                          <Button type="submit">{t("guardrailsPage.details.saveChanges")}</Button>
                        </div>
                      </FieldGroup>
                    </form>
                  </TooltipProvider>
                  ...
                  <div className="space-y-4">
                    <div>
                      <p className="font-medium">{t("guardrailsPage.columns.id")}</p>
                      <div className="font-mono">{guardrailData.guardrail_id}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("guardrailsPage.details.guardrailName")}</p>
                      <div>{guardrailData.guardrail_name || t("guardrailsPage.values.unnamed")}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("guardrailsPage.columns.provider")}</p>
                      <div>{displayName}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("guardrailsPage.columns.mode")}</p>
                      <div>{formatGuardrailMode(guardrailData.litellm_params?.mode) || "-"}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("guardrailsPage.columns.defaultOn")}</p>
                      <Badge variant={guardrailData.litellm_params?.default_on ? "secondary" : "outline"}>
                        {guardrailData.litellm_params?.default_on
                    ...
                    <div>
                      <p className="font-medium">{t("guardrailsPage.columns.createdAt")}</p>
                      <div>{formatDate(guardrailData.created_at)}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("guardrailsPage.details.lastUpdated")}</p>
