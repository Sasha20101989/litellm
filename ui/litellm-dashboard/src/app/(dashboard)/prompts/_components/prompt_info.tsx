...
const PromptInfoView: React.FC<PromptInfoProps> = ({ promptId, onClose, accessToken, isAdmin, onDelete, onEdit }) => {
  ...
  const fetchPromptInfo = async (environment?: string) => {
    try {
      ...
      NotificationsManager.fromBackend(t("details.loadFailed"));
  ...
  if (loading && !promptData) {
    return <div className="p-4">{t("details.loading")}</div>;
  }

  if (!promptData) {
    return <div className="p-4">{t("details.notFound")}</div>;
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("list.deleted", { name: basePromptId }));
      onDelete?.();
      onClose();
    } catch (error) {
  ...
  const handleVersionClick = async (version: PromptSpec) => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("details.versionLoadFailed", { version: versionNum }));
    }
  };
    ...
    <div className="p-4">
      ...
      {isViewingOldVersion && (
        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center justify-between">
          <Text className="text-amber-800">
            {t("details.oldVersion", { current: selectedVersion, latest: latestVersion })}
          </Text>
          <TremorButton
            variant="light"
            size="xs"
      ...
      <TabGroup>
        <TabList className="mb-4">
          <Tab key="overview">{t("details.overview")}</Tab>
          {promptTemplate ? <Tab key="prompt-template">{t("details.template")}</Tab> : <></>}
          <Tab key="raw-json">{t("details.rawJson")}</Tab>
        </TabList>
        ...
        <TabPanels>
          ...
          <TabPanel>
            <Grid numItems={1} numItemsSm={2} numItemsLg={4} className="gap-4">
              ...
              <Card>
                <Text>{t("details.promptType")}</Text>
                <div className="mt-2">
              ...
              <Card>
                <Text>{t("details.createdAt")}</Text>
                <div className="mt-2">
                  <Title className="text-sm">{formatDate(promptData.created_at)}</Title>
                  <Text className="text-xs">{t("details.updatedAt", { date: formatDate(promptData.updated_at) })}</Text>
                </div>
              </Card>
            </Grid>
            ...
            <Card className="mt-6">
              ...
              {loadingVersions ? (
                <Text>{t("details.loadingVersions")}</Text>
              ) : versionHistory.length > 0 ? (
                <Table>
                  <TableHead>
                    <TableRow>
                      <TableHeaderCell>{t("details.version")}</TableHeaderCell>
                      <TableHeaderCell>{t("details.createdBy")}</TableHeaderCell>
                      <TableHeaderCell>{t("details.date")}</TableHeaderCell>
                      <TableHeaderCell>{t("details.actions")}</TableHeaderCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
            ...
            <TabPanel>
              <Card>
                <div className="flex justify-between items-center mb-4">
                  <Title>{t("details.template")}</Title>
                  <Button
                    type="text"
                    size="small"
                  ...
                  >
                    {copiedStates["prompt-content"] ? t("details.copied") : t("details.copyContent")}
                  </Button>
                </div>

                <div className="space-y-4">
                  <div>
                    <Text className="font-medium">{t("details.templateId")}</Text>
                  ...
                  <div>
                    <Text className="font-medium">{t("details.content")}</Text>
                  ...
                  {promptTemplate.metadata && Object.keys(promptTemplate.metadata).length > 0 && (
                    <div>
                      <Text className="font-medium">{t("details.metadata")}</Text>
          ...
          <TabPanel>
            <Card>
              <div className="flex justify-between items-center mb-4">
                <Title>{t("details.rawResponse")}</Title>
                <Button
                  type="text"
                  size="small"
                ...
                >
                  {copiedStates["raw-json"] ? t("details.copied") : t("details.copyJson")}
                </Button>
              </div>
