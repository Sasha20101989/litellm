...
const UIThemeSettings: React.FC<UIThemeSettingsProps> = ({ userID, userRole, accessToken }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        NotificationsManager.success(t("theme.updated"));
      ...
      NotificationsManager.fromBackend(t("theme.updateFailed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (response.ok) {
        NotificationsManager.success(t("theme.resetSuccess"));
      } else {
        throw new Error("Failed to reset");
      }
    } catch (error) {
      console.error("Error resetting theme settings:", error);
      NotificationsManager.fromBackend(t("theme.resetFailed"));
    } finally {
      setLoading(false);
    }
  };
