...
export const VectorStoreTester: React.FC<VectorStoreTesterProps> = ({ vectorStoreId, accessToken, className = "" }) => {
  ...
  const handleSearch = async () => {
    if (!query.trim()) {
      toast.warning(t("vectorStores.test.queryRequired"));
      return;
    }
  ...
  const clearHistory = () => {
    setSearchHistory([]);
    setExpandedResults({});
    toast.success(t("vectorStores.test.historyCleared"));
  };
      ...
      <div className="flex h-150 flex-col">
        ...
        <div className="flex items-center justify-between border-b p-4">
          <div className="flex items-center">
            ...
            <h4 className="text-base font-medium text-foreground">{t("vectorStores.test.title")}</h4>
          </div>
          {searchHistory.length > 0 && (
            <Button variant="outline" size="sm" onClick={clearHistory}>
              {t("vectorStores.test.clearHistory")}
            </Button>
          )}
        </div>
        ...
        <div className="flex-1 overflow-auto p-4 pb-0">
          {searchHistory.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="text-sm">{t("vectorStores.test.empty")}</p>
            </div>
          ) : (
                  ...
                  <div className="text-right">
                    <div className="inline-block max-w-[80%] rounded-lg bg-muted p-3 shadow-xs ring-1 ring-foreground/10">
                      <div className="mb-1 flex items-center gap-2">
                        <strong className="text-sm">{t("vectorStores.test.query")}</strong>
                  ...
                  <div className="text-left">
                    <div className="inline-block max-w-[80%] rounded-lg bg-card p-3 shadow-xs ring-1 ring-foreground/10">
                      <div className="mb-2 flex items-center gap-2">
                        ...
                        <strong className="text-sm">{t("vectorStores.test.results")}</strong>
                        {entry.response && (
                          <span className="rounded-sm bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                            {t("vectorStores.test.resultCount", { count: entry.response.data?.length || 0 })}
                          </span>
                        )}
                      </div>
                      ...
                      {entry.response && entry.response.data && entry.response.data.length > 0 ? (
                              ...
                              <div key={resultIndex} className="overflow-hidden rounded-lg border bg-muted/50">
                                ...
                                >
                                  <div className="flex items-center">
                                    ...
                                    <span className="text-sm font-medium">{t("vectorStores.test.result", { number: resultIndex + 1 })}</span>
                                  ...
                                  <span className="rounded-sm bg-muted px-2 py-1 text-xs text-foreground">
                                    {t("vectorStores.test.score", { score: result.score.toFixed(4) })}
                                  </span>
                                </div>
                                  ...
                                  <div className="border-t bg-card p-3">
                                      ...
                                      result.content.map((content, contentIndex) => (
                                        <div key={contentIndex} className="mb-3">
                                          <div className="mb-1 text-xs text-muted-foreground">
                                            {t("vectorStores.test.content", { type: content.type })}
                                          </div>
                                    ...
                                    {(result.file_id || result.filename || result.attributes) && (
                                      <div className="mt-3 border-t pt-3">
                                        <div className="mb-2 text-xs font-medium text-muted-foreground">{t("vectorStores.test.metadata")}</div>
                                        <div className="space-y-2 text-xs">
                                          {result.file_id && (
                                            <div className="rounded-sm bg-muted/50 p-2">
                                              <span className="font-medium">{t("vectorStores.test.fileId")}</span> {result.file_id}
                                            </div>
                                          )}
                                          {result.filename && (
                                            <div className="rounded-sm bg-muted/50 p-2">
                                              <span className="font-medium">{t("vectorStores.test.filename")}</span> {result.filename}
                                            </div>
                                          )}
                                          {result.attributes && Object.keys(result.attributes).length > 0 && (
                                            <div className="rounded-sm bg-muted/50 p-2">
                                              <span className="mb-1 block font-medium">{t("vectorStores.test.attributes")}</span>
                        ...
                        >
                          {entry.error ? `${t("vectorStores.test.searchFailed")}: ${entry.error}` : t("vectorStores.test.noResults")}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="flex items-end space-x-2">
            <div className="flex-1">
              <Textarea
                ...
                placeholder={t("vectorStores.test.placeholder")}
                disabled={isLoading}
                rows={1}
