...
const VectorStoreManagement: React.FC<VectorStoreProps> = ({ accessToken, userID, userRole, isViewOnly }) => {
  ...
  const fetchVectorStores = async () => {
    ...
    try {
      ...
      toast.fromError(t("vectorStores.fetchFailed", { error: String(error) }));
    } finally {
      setIsLoadingVectorStores(false);
    }
  };
  ...
  const fetchCredentials = async () => {
    ...
    try {
      ...
      toast.fromError(t("vectorStores.credentialsFetchFailed", { error: String(error) }));
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("vectorStores.deleted"));
      fetchVectorStores();
    } catch (error) {
      console.error("Error deleting vector store:", error);
      toast.fromError(t("vectorStores.deleteFailed", { error: String(error) }));
    } finally {
      setIsDeleting(false);
      ...
      <div className="gap-2 p-8 w-full mt-2">
        <div className="flex justify-between mt-2 w-full items-center mb-4">
          ...
          <div className="flex items-center space-x-2">
            {lastRefreshed && <p className="text-sm text-muted-foreground">{t("vectorStores.lastRefreshed", { time: lastRefreshed })}</p>}
            <Button variant="outline" size="icon-sm" aria-label={t("vectorStores.refresh")} onClick={handleRefreshClick}>
